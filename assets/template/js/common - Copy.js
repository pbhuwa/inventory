...
function check_form_validation(){
  var reqlength = $('.required_field').length;

  var value = $('.required_field').filter(function () {
  ...
  if (value.length >= 0  && (value.length !== reqlength)) {
    ...
    $(document).on('click','.save',function(e){
      ...
      var isvalid = check_form_validation();
...
$(document).on('click','.overview',function(){
      ...
      $.redirectPost(redirecturl, {equipkey:equipkey});
      return false;
    })
  ...
  // $('.searchText').keydown(function (e) {  
    ...
    switch (parseInt(e.keyCode))
    {
      ...
      var cur = $('#DisplayBlock_'+attrid +' .flatTable tbody').find('tr.current');
        ...
        if (e.keyCode == '13' || e.keyCode == '18' || e.keyCode == '17' || e.keyCode == '40' ||
          e.keyCode == '38' || e.keyCode == '39' || e.keyCode == '37' || e.keyCode == '9')
        {
    ...
    $.ajax({
      type: "POST",
      url: srchurl,
      data:{srchtext:srchtext,srchdec:srchdec,serialno:serialno},
      dataType: 'html',
      beforeSend: function() {
      // $('.overlay').modal('show');
    },
...
$(document).on('click','.btnpmdata',function(){
  ...
  $.ajax({
    type: "POST",
    url: url,
    data:{pmcount:pmcount,month:month,year:year,week:week,date:date,type:type,pmcat:pmcat},
    dataType: 'html',
    beforeSend: function() {
      $('.overlay').modal('show');
    },
...
$(document).on('click','.generate_export_file',function(){
 ...
 if(type == 'excel'){
  ...
  window.open(base_url + moduleLocation + '/?'+$.param($(tableid).DataTable().ajax.params()),'_blank');
}  
});
    ...
    function model_keypress(){
      ...
      $.singleDoubleClick = function(singleClk, doubleClk) {
        ...
        $.redirectPost(redirecturl, {mid:mid,id:id,detailid:detailid,date:date});
      })
...
modified_substring = modified_substring.replace( /</g , "?" )   ;  
  ...
  $(document).on('click','.searchReport',function() {
    ...
    var displayid = $(this).data('displayid');
        ...
        var formid=$('.searchReport').closest('form').attr('id');
  ...
  $(document).on('click','.btn_gen_report',function(){
    ...
    var itemid= $('#itemid').val();
      ...
      $.redirectPost(redirecturl, {departmentid:departmentid,store_id:store_id,reqno:reqno,fromdate:fromdate,todate:todate,fyear:fyear,locationid:locationid, fyear:fyear, store_id:store_id, materialtype:materialtype, depid:depid, catid:catid, is_summary:is_summary, userid:userid, supplierid:supplierid, itemid:itemid, asen_assettype:asen_assettype, asen_manufacture:asen_manufacture, asen_status:asen_status, asen_condition:asen_condition, asen_depreciation:asen_depreciation});
  ...
  $(document).on('click','.btnredirect',function(){
    ...
    var redirecturl=url;
    $.redirectPost(redirecturl, {id:id, otherdata:otherdata, dashboard_data:dashboard_data });
  });
