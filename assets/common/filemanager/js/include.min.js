...
{
	var t=$(".breadcrumb").width()+e;$(".uploader").css("width",t);if($("#view").val()>0)
	{
		if($("#view").val()==1)
			{
				$("ul.grid li, ul.grid figure").css("width","100%")}else{var n=Math.floor(t/380);
					if(n==0)
						{
							n=1;$("h4").css("font-size",12)}t=Math.floor(t/n-3);
							$("ul.grid li, ul.grid figure").css("width",t)}$("#help").hide()
						}else
						{
							if(Modernizr.touch)
								{
...
function swipe_reaction(e,t,n,r,i){var s=$(this);if($("#view").val()==0){if(s.attr("toggle")==1){s.attr("toggle",0);s.animate({top:"0px"},{queue:false,duration:300})}else{s.attr("toggle",1);s.animate({top:"-30px"},{queue:false,duration:300})}}}

function apply(e,t)
{
	if($("#popup").val()==1)var n=window.opener;
	else var n=window.parent;var r=$("#cur_dir").val();
	var i=$("#base_url").val();
	var s=e.substr(0,e.lastIndexOf("."));
	var o=e.split(".").pop();o=o.toLowerCase();
	var u="";if($.inArray(o,ext_img)>-1)
	{
		u='<img src="'+i+r+e+'" alt="'+s+'" />'
			...
			parent.tinymce.activeEditor.insertContent(u);parent.tinymce.activeEditor.windowManager.close()}

function apply_link(e,t){
	if($("#popup").val()==1)
		var n=window.opener;
	else var n=window.parent;
	var r=$("#cur_dir").val();r=r.replace("\\","/");
	var i=$("#base_url").val();if(t!="")
	{
		var s=$("#"+t,n.document);
		$(s).val(i+r+e);
		close_window()
