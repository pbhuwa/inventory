...
<form method="post" id="FormIssuehandover" action="<?php echo base_url('handover/handover_issue/save_handover_issue'); ?>" data-reloadurl="<?php echo base_url('handover/handover_issue/form_handover_issue');?>" class="form-material form-horizontal form">

    <input type="hidden" name="id" value="<?php echo !empty($new_issue[0]->sama_salemasterid)?$new_issue[0]->sama_salemasterid:'';  ?>">

    <div id="issueDetails">
        <div class="form-group">
            <!-- <input type="hidden" name="locationid" id="tolocid" value=""> -->
            <div class="col-md-3 col-sm-4">
                <label>Location To</label>
            <select name="locationid" id="locationid" class="form-control" >
                <option value="">--Select--</option>
            <?php
             $db_location = $this->general->get_tbl_data('*', 'loca_location', array('loca_isactive' => 'Y'), 'loca_locationid', 'ASC'); 
             // echo "<pre>";
             // print_r($db_location);
             // die();

             if(!empty($db_location)){
                foreach ($db_location as $kl => $loc) {
                   ?>
        ...
        <!--  <?php echo $this->general->location_option(3,'','locationid',false,' To'); ?> -->
    </div>
        <div class="col-md-3 col-sm-4">
      ...
      <!--   <?php if($reqno){
        ...
        ?> -->
        <?php if($reqno){
            $handover_no = !empty($reqno)?$reqno:'';
        }else{
            $handover_no = !empty($new_issue[0]->harm_handoverreqno)?$new_issue[0]->harm_handoverreqno:'';
        }
        ?>
        <label for="example-text"><?php //echo $this->lang->line('req_no'); ?>Handover Req. No. <span class="required">*</span>:</label>
        <div class="dis_tab">
            <input type="text" class="form-control reqno send_after_stop number enterinput required_field " name="sama_requisitionno" value="<?php echo $handover_no; ?>" placeholder="Enter Handover Req. No" id="req_no" data-targetbtn="SrchReq" autocomplete="off" autofocus="on">
            <a href="javascript:void(0)" class="table-cell width_30 btn btn-success"  id="SrchReq"><i class="fa fa-search"></i></a>&nbsp;
            <!-- <a href="javascript:void(0)"  data-id="0" data-fyear="<?php echo CUR_FISCALYEAR;?>" data-displaydiv="Issue" data-viewurl="<?php echo base_url() ?>handover/handover_req/load_requisition" class="view table-cell width_30 btn btn-success" data-heading="<?php echo $this->lang->line('load_handover')?>" id="reqload" ><i class="fa fa-upload"></i></a> -->
        </div>
    </div>
</div>
...
<div class="form-group">
    ...
    <div class="col-md-3 col-sm-4">
        ...
        ?>
        <label for="example-text"><?php echo $this->lang->line('handover_issue_date'); ?><span class="required">*</span> :</label>
    ...
    <div class="col-md-3 col-sm-4">
        <label for="example-text"><?php echo $this->lang->line('handover_req_date'); ?><span class="required">*</span>  :</label>
    ...
    <div class="col-md-3 col-sm-4">
        <?php $sama_invoiceno=!empty($new_issue[0]->haov_handoverno)?$new_issue[0]->haov_handoverno:''; ?>
        <label for="example-text"><?php echo $this->lang->line('handover_issue_no'); ?> :</label>
        <input type="text" class="form-control" name="sama_invoiceno" id="issue_no"  value="<?php echo !empty($sama_invoiceno)?$sama_invoiceno:$handoverissue_no; ?>" placeholder="Enter Handover Issue Number" readonly>
    </div>
...
<script type="text/javascript">
   ...
   function getPendingList(req_masterid, main_form=false){
    if(main_form == 'main_form'){
        var submiturl = base_url+'handover/handover_issue/load_pendinglist/handover_issue_pending_list';
        var displaydiv = '#DisplayPendingList'; 
    }else{
        var submiturl = base_url+'handover/handover_req/load_pendinglist';
...
function ajax_search_requistion()
{
    ...
    var submitdata = {req_no:req_no,fyear:fyear,locationid:locationid};
    var submiturl = base_url+'handover/handover_issue/handover_issuelist_by_req_no';
    ...
    function onSuccess(jsons){
        ...
        if(data.status=='success')
        {
            ...
            $('#depid').val(reqdata.fromdepid);
            $('#depnme').val(reqdata.fromdepid);
            $('#tolocid').val(reqdata.fromlocationid);
            $('#locationid').val(reqdata.fromlocationid);
