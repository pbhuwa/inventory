...
class Treatmentplantype_mdl extends CI_Model 

{
...

	{
		...
		$this->table='tept_treatmentplantype';
	...
	public $validate_settings_valve = array( 
	  ...
	  array('field' => 'tept_name', 'label' => 'Treatmentplant Type', 'rules' => 'trim|required|xss_clean|callback_exists_treatmentplantype'),
...

	{
		...
		$id=$this->input->post('tept_trplid');
		...
		if($id)
		{
			...
			if(!empty($postdata))
			{
		        $this->general->save_log($this->table,'tept_treatmentplantype',$id,$postdata,'Update');
		        $this->db->update($this->table,$postdata,array('tept_trplid'=>$id));
			}
	...
	public function get_all_tretmentplan($srchcol=false)

	{

		$this->db->select('tp.*');

		 $this->db->from('tept_treatmentplantype tp');
...

	{
...

		{
...

			{
				 ...
				 $this->general->save_log($this->table,'tept_trplid',$id,$postdata,'Update');
...

		{
...

			{
				...
				$this->db->insert('tept_treatmentplantype',$postdata);
...

	{
...

		{
			 ...
			 $this->general->save_log($this->table,'tept_trplid',$id,$postdata=array(),'Delete');

			$this->db->delete($this->table,array('tept_trplid'=>$id));
...

	{
		...
		$get = $_GET;
     	...
     	if(!empty($get['sSearch_0'])){
            ...
            $this->db->where("tept_trplid like  '%".$get['sSearch_0']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("tept_name like  '%".$get['sSearch_1']."%'  ");

        }
        if(!empty($get['sSearch_2'])){

            $this->db->where("tept_isactive like  '%".$get['sSearch_2']."%'  ");
		...
		$this->db->from('tept_treatmentplantype');
...
public function save_equipment_cat()
	...
	public function check_exist_treatment_for_other($treatmentptype = false, $id = false){
...

		{
			...
			$query = $this->db->get_where($this->table,array('tept_name'=>$treatmentptype,'tept_trplid !='=>$id));
...

		{
			...
			$query = $this->db->get_where($this->table,array('tept_name'=>''));
