...
class Pavement_mdl extends CI_Model 

{
	...
	public $validate_settings_pavement = array( 

	  array('field' => 'paty_name', 'label' => 'pavement Name', 'rules' => 'trim|required|xss_clean|callback_exists_pavementtype'),
...

	{
		...
		$this->general->save_log($this->table,'paty_pavementtype',$id,$postdata,'Update');

		$this->db->update($this->table,$postdata,array('paty_id'=>$id));
	...
	public function get_all_pavements($srchcol=false)

	{

		$this->db->select('mt.*');

		 $this->db->from('paty_pavementtype mt');
	...
	public function get_all_pavement_list($srchcol=false)
...

	{
...

		{
...

			{
				 ...
				 $this->general->save_log($this->table,'paty_id',$id,$postdata,'Update');
...

		{
...

			{
				...
				$this->db->insert('paty_pavementtype',$postdata);
	...
	public function remove_pavement()

	{
...

		{
			 ...
			 $this->general->save_log($this->table,'paty_id',$id,$postdata=array(),'Delete');

			$this->db->delete($this->table,array('paty_id'=>$id));
...

	{
		...
		$get = $_GET;
     	...
     	if(!empty($get['sSearch_0'])){
            ...
            $this->db->where("paty_id like  '%".$get['sSearch_0']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("paty_name like  '%".$get['sSearch_1']."%'  ");

        }
        if(!empty($get['sSearch_2'])){

            $this->db->where("paty_isactive like  '%".$get['sSearch_2']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("paty_typw like  '%".$get['sSearch_1']."%'  ");
		...
		$this->db->from('paty_pavementtype');
...
public function save_equipment_cat()
	...
	public function check_exist_pavement_for_other($pavementtype = false, $id = false){
...

		{
			...
			$query = $this->db->get_where($this->table,array('paty_name'=>$pavementtype,'paty_id !='=>$id));
...

		{
			...
			$query = $this->db->get_where($this->table,array('paty_name'=>$s));
