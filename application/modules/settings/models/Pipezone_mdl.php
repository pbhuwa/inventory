...
class Pipezone_mdl extends CI_Model 

{
	...
	public $validate_settings_pipezone = array( 

	  array('field' => 'pizo_name', 'label' => 'Pipezone', 'rules' => 'trim|required|xss_clean|callback_exists_pipezonetype'),
...

	{
		...
		$this->general->save_log($this->table,'pizo_pipezone',$id,$postdata,'Update');

		$this->db->update($this->table,$postdata,array('pizo_id'=>$id));
	...
	public function get_all_pipezones($srchcol=false)

	{

		$this->db->select('mt.*');

		 $this->db->from('pizo_pipezone mt');
	...
	public function get_all_pipezone_list($srchcol=false)
...

	{
...

		{
...

			{
				 ...
				 $this->general->save_log($this->table,'pizo_id',$id,$postdata,'Update');
...

		{
...

			{
				...
				$this->db->insert('pizo_pipezone',$postdata);
	...
	public function remove_pipezone()

	{
...

		{
			 ...
			 $this->general->save_log($this->table,'pizo_id',$id,$postdata=array(),'Delete');

			$this->db->delete($this->table,array('pizo_id'=>$id));
...

	{
		...
		$get = $_GET;
     	...
     	if(!empty($get['sSearch_0'])){
            ...
            $this->db->where("pizo_id like  '%".$get['sSearch_0']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("pizo_name like  '%".$get['sSearch_1']."%'  ");

        }
        if(!empty($get['sSearch_2'])){

            $this->db->where("pizo_isactive like  '%".$get['sSearch_2']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("pizo_typw like  '%".$get['sSearch_1']."%'  ");
...
public function save_equipment_cat()
	...
	public function check_exist_pipezone_for_other($pipezonetype = false, $id = false){
...

		{
			...
			$query = $this->db->get_where($this->table,array('pizo_name'=>$pipezonetype,'pizo_id !='=>$id));
...

		{
			...
			$query = $this->db->get_where($this->table,array('pizo_name'=>$s));
