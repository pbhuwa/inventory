...
class Pipematerial_mdl extends CI_Model 

{
...

	{
		...
		$this->table='pimt_pipematerialtype';
	...
	public $validate_settings_pipematerial = array( 

	  array('field' => 'pimt_name', 'label' => 'Pipe Material Type', 'rules' => 'trim|required|xss_clean|callback_exists_pipematerialtype'),
...

	{
		...
		$this->general->save_log($this->table,'pimt_pipematerialtype',$id,$postdata,'Update');

		$this->db->update($this->table,$postdata,array('pimt_id'=>$id));
	...
	public function get_all_pipematerials($srchcol=false)

	{

		$this->db->select('mt.*');

		 $this->db->from('pimt_pipematerialtype mt');
	...
	public function get_all_pipematerial_list($srchcol=false)
...

	{
...

		{
...

			{
				 ...
				 $this->general->save_log($this->table,'pimt_id',$id,$postdata,'Update');
...

		{
...

			{
				...
				$this->db->insert('pimt_pipematerialtype',$postdata);
	...
	public function remove_pipematerial()

	{
...

		{
			 ...
			 $this->general->save_log($this->table,'pimt_id',$id,$postdata=array(),'Delete');

			$this->db->delete($this->table,array('pimt_id'=>$id));
...

	{
		...
		$get = $_GET;
     	...
     	if(!empty($get['sSearch_0'])){
            ...
            $this->db->where("pimt_id like  '%".$get['sSearch_0']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("pimt_name like  '%".$get['sSearch_1']."%'  ");

        }
        if(!empty($get['sSearch_2'])){

            $this->db->where("pimt_isactive like  '%".$get['sSearch_2']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("pimt_typw like  '%".$get['sSearch_1']."%'  ");
		...
		$this->db->from('pimt_pipematerialtype');
...
public function save_equipment_cat()
	...
	public function check_exist_pipematerial_for_other($pipematerialtype = false, $id = false){
...

		{
			...
			$query = $this->db->get_where($this->table,array('pimt_name'=>$pipematerialtype,'pimt_id !='=>$id));
...

		{
			...
			$query = $this->db->get_where($this->table,array('pimt_name'=>$s));
