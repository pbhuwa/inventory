...
class Valve_mdl extends CI_Model 

{
	...
	public $validate_settings_valve = array( 
	  ...
	  array('field' => 'vaty_name', 'label' => 'Valve Type', 'rules' => 'trim|required|xss_clean|callback_exists_valvetype'),
...

	{
		...
		$this->general->save_log($this->table,'vaty_valvetype',$id,$postdata,'Update');

		$this->db->update($this->table,$postdata,array('vaty_id'=>$id));
	...
	public function get_all_valves($srchcol=false)

	{

		$this->db->select('mt.*');

		 $this->db->from('vaty_valvetype mt');
...

	{
...

		{
...

			{
				 ...
				 $this->general->save_log($this->table,'vaty_id',$id,$postdata,'Update');
...

		{
...

			{
				...
				$this->db->insert('vaty_valvetype',$postdata);
...

	{
...

		{
			 ...
			 $this->general->save_log($this->table,'vaty_id',$id,$postdata=array(),'Delete');

			$this->db->delete($this->table,array('vaty_id'=>$id));
...

	{
		...
		$get = $_GET;
     	...
     	if(!empty($get['sSearch_0'])){
            ...
            $this->db->where("vaty_id like  '%".$get['sSearch_0']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("vaty_name like  '%".$get['sSearch_1']."%'  ");

        }
        if(!empty($get['sSearch_2'])){

            $this->db->where("vaty_isactive like  '%".$get['sSearch_2']."%'  ");
...
public function save_equipment_cat()
	...
	public function check_exist_valve_for_other($valvetype = false, $id = false){
...

		{
			...
			$query = $this->db->get_where($this->table,array('vaty_name'=>$valvetype,'vaty_id !='=>$id));
...

		{
			...
			$query = $this->db->get_where($this->table,array('vaty_name'=>$s));
