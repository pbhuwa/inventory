...
class Hydrant_mdl extends CI_Model 

{
	...
	public $validate_settings_hydrant = array( 

	  array('field' => 'hyty_name', 'label' => 'hydrant Type', 'rules' => 'trim|required|xss_clean|callback_exists_hydranttype'),
...

	{
		...
		$this->general->save_log($this->table,'hyty_hydrantstype',$id,$postdata,'Update');

		$this->db->update($this->table,$postdata,array('hyty_id'=>$id));
	...
	public function get_all_hydrants($srchcol=false)

	{

		$this->db->select('mt.*');

		 $this->db->from('hyty_hydrantstype mt');
	...
	public function get_all_hydrant_list($srchcol=false)
...

	{
...

		{
...

			{
				 ...
				 $this->general->save_log($this->table,'hyty_id',$id,$postdata,'Update');
...

		{
...

			{
				...
				$this->db->insert('hyty_hydrantstype',$postdata);
	...
	public function remove_hydrant()

	{
...

		{
			 ...
			 $this->general->save_log($this->table,'hyty_id',$id,$postdata=array(),'Delete');

			$this->db->delete($this->table,array('hyty_id'=>$id));
...

	{
		...
		$get = $_GET;
     	...
     	if(!empty($get['sSearch_0'])){
            ...
            $this->db->where("hyty_id like  '%".$get['sSearch_0']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("hyty_name like  '%".$get['sSearch_1']."%'  ");

        }
        if(!empty($get['sSearch_2'])){

            $this->db->where("hyty_isactive like  '%".$get['sSearch_2']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("hyty_typw like  '%".$get['sSearch_1']."%'  ");
		...
		$this->db->from('hyty_hydrantstype');
...
public function save_equipment_cat()
	...
	public function check_exist_hydrant_for_other($hydranttype = false, $id = false){
...

		{
			...
			$query = $this->db->get_where($this->table,array('hyty_name'=>$hydranttype,'hyty_id !='=>$id));
...

		{
			...
			$query = $this->db->get_where($this->table,array('hyty_name'=>$s));
