...
class Buildingtype_mdl extends CI_Model 

{
	...
	public $validate_settings_build = array( 

	  array('field' => 'buty_name', 'label' => 'Building Type Name', 'rules' => 'trim|required|xss_clean|callback_exists_buildtype'),
...

	{
		...
		$id=$this->input->post('buty_butyid');
		...
		$this->general->save_log($this->table,'buty_buildingtype',$id,$postdata,'Update');

		$this->db->update($this->table,$postdata,array('buty_butyid'=>$id));
	...
	public function get_all_builds($srchcol=false)

	{

		$this->db->select('bt.*');

		 $this->db->from('buty_buildingtype bt');
	...
	// public function get_all_build_list($srchcol=false)
...

	{
		...
		$postdata=$this->input->post();
		$postdata['buty_locationid']=$this->locationid;
		$postdata['buty_postip']=$this->ip;
		$postdata['buty_posttime']=$this->curtime;
		$postdata['buty_postmac']=$this->mac;
		$postdata['buty_orgid']=$this->orgid;
...

		{
...

			{
				 ...
				 $this->general->save_log($this->table,'buty_butyid',$id,$postdata,'Update');
				$this->db->update($this->table,$postdata,array('buty_butyid'=>$id));	
...

		{
...

			{
				...
				$this->db->insert('buty_buildingtype',$postdata);
...

	{
...

		{
			 ...
			 $this->general->save_log($this->table,'buty_butyid',$id,$postdata=array(),'Delete');

			$this->db->delete($this->table,array('buty_butyid'=>$id));
...

	{
		$get = $_GET;
     	...
     	if(!empty($get['sSearch_0'])){
            ...
            $this->db->where("buty_butyid like  '%".$get['sSearch_0']."%'  ");

        }

        if(!empty($get['sSearch_1'])){

            $this->db->where("buty_name like  '%".$get['sSearch_1']."%'  ");

        }

        if(!empty($get['sSearch_2'])){

            $this->db->where("buty_isactive like  '%".$get['sSearch_2']."%'  ");
		...
		$this->db->from('buty_buildingtype');
...
public function save_equipment_cat()
	...
	public function check_exist_build_for_other($buildtype = false, $id = false){
...

		{
			...
			$query = $this->db->get_where($this->table,array('buty_name'=>$buildtype,'buty_butyid !='=>$id));
...

		{
			...
			$query = $this->db->get_where($this->table,array('buty_name'=>''));
