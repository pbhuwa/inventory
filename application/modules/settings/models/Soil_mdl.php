...
class Soil_mdl extends CI_Model 

{
	...
	public $validate_settings_soil = array( 

	  array('field' => 'soty_type', 'label' => 'Soil Type', 'rules' => 'trim|required|xss_clean|callback_exists_soiltype'),
...

	{
		...
		$this->general->save_log($this->table,'soty_soiltype',$id,$postdata,'Update');

		$this->db->update($this->table,$postdata,array('soty_id'=>$id));
	...
	public function get_all_soils($srchcol=false)

	{

		$this->db->select('mt.*');

		 $this->db->from('soty_soiltype mt');
	...
	public function get_all_soil_list($srchcol=false)
...

	{
...

		{
...

			{
				 ...
				 $this->general->save_log($this->table,'soty_id',$id,$postdata,'Update');
...

		{
...

			{
				...
				$this->db->insert('soty_soiltype',$postdata);
...

	{
...

		{
			 ...
			 $this->general->save_log($this->table,'soty_id',$id,$postdata=array(),'Delete');

			$this->db->delete($this->table,array('soty_id'=>$id));
...

	{
		...
		$get = $_GET;
     	...
     	if(!empty($get['sSearch_0'])){
            ...
            $this->db->where("soty_id like  '%".$get['sSearch_0']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("soty_type like  '%".$get['sSearch_1']."%'  ");

        }
        if(!empty($get['sSearch_2'])){

            $this->db->where("soty_isactive like  '%".$get['sSearch_2']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("soty_typw like  '%".$get['sSearch_1']."%'  ");
...
public function save_equipment_cat()
	...
	public function check_exist_soil_for_other($soiltype = false, $id = false){
...

		{
			...
			$query = $this->db->get_where($this->table,array('soty_type'=>$soiltype,'soty_id !='=>$id));
...

		{
			...
			$query = $this->db->get_where($this->table,array('soty_type'=>$s));
