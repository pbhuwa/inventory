...
class Desposal_type_mdl extends CI_Model 

{
	...
	public $validate_settings_valve = array( 
	  ...
	  array('field' => 'dety_name', 'label' => 'Desposal Type', 'rules' => 'trim|required|xss_clean|callback_exists_desposaltype'),
...

	{
		...
		$id=$this->input->post('dety_detyid');
		...
		if($id)
		{
			...
			if(!empty($postdata))
			{
		        $this->general->save_log($this->table,'dety_desposaltype',$id,$postdata,'Update');
		        $this->db->update($this->table,$postdata,array('dety_detyid'=>$id));
			}
	...
	public function get_all_desposal($srchcol=false)

	{

		$this->db->select('dt.*');

		 $this->db->from('dety_desposaltype dt');
	...
	// 	$this->db->from('dety_desposaltype as');
...

	{
...

		{
...

			{
				 ...
				 $this->general->save_log($this->table,'dety_detyid',$id,$postdata,'Update');
...

		{
...

			{
				...
				$this->db->insert('dety_desposaltype',$postdata);
	...
	public function remove_desposal()

	{
...

		{
			 ...
			 $this->general->save_log($this->table,'dety_detyid',$id,$postdata=array(),'Delete');

			$this->db->delete($this->table,array('dety_detyid'=>$id));
...

	{
		...
		$get = $_GET;
     	...
     	if(!empty($get['sSearch_0'])){
            ...
            $this->db->where("dety_detyid like  '%".$get['sSearch_0']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("dety_name like  '%".$get['sSearch_1']."%'  ");

        }
        if(!empty($get['sSearch_2'])){

            $this->db->where("dety_isactive like  '%".$get['sSearch_2']."%'  ");
		...
		$this->db->from('dety_desposaltype');
...
public function save_equipment_cat()
	...
	public function check_exist_status_for_other($despotype = false, $id = false){
...

		{
			...
			$query = $this->db->get_where($this->table,array('dety_name'=>$despotype,'dety_detyid !='=>$id));
...

		{
			...
			$query = $this->db->get_where($this->table,array('dety_name'=>''));
