...
class Receive_against_order_mdl extends CI_Model
{
    ...
    public $validate_settings_cancelreceive = array(
     ...
     public $validate_settings_receive_against_order = array(
       ...
       public $validate_settings_receive_against_order_no_supplier = array(
        array('field' => 'supplierid[]', 'label' => 'Supplier Name', 'rules' => 'trim|required|xss_clean'),
        array('field' => 'received_no', 'label' => 'Received No', 'rules' => 'trim|required|xss_clean'),
        array('field' => 'bill_no[]', 'label' => 'Supplier Bill No.', 'rules' => 'trim|required|xss_clean'),
        array('field' => 'bill_date[]', 'label' => 'Supplier Bill Date.', 'rules' => 'trim|required|xss_clean'),
        array('field' => 'received_date', 'label' => 'Received Date', 'rules' => 'trim|required|xss_clean'),
        array('field' => 'received_qty[]', 'label' => 'Received Qty', 'rules' => 'trim|required|xss_clean|greater_than[0]'),
    ...
    public function get_receive_against_order_list_ku($cond = false)
    {
        ...
        $pur_type = !empty($get['pur_type']) ? $get['pur_type'] : $this->input->post('pur_type');
        ...
        if (!empty($pur_type)) {
            if ($pur_type == 'order_received') {
                $this->db->where('recm_purchaseordermasterid != 0 OR recm_challanno != 0');
            } elseif ($pur_type == 'direct_purchase') {
        ...
        if ($this->input->get('iSortCol_0') == 1)
            ...
            $order_by = 'recm_clearanceamount';
             ...
             if(DEFAULT_DATEPICKER == 'NP'){
                $order_by = 'recm_postdatebs';
            }else{
                $order_by = 'recm_postdateas';
            }
        $totalrecs = '';
        $limit = 15;
        $offset = 1;
        ...
        if (!empty($pur_type)) {
            if ($pur_type == 'order_received') {
                $this->db->where('recm_purchaseordermasterid != 0 OR recm_challanno != 0');
            } else if ($pur_type == 'direct_purchase') {
                $this->db->where('rm.recm_dstat', 'D');
            }
        }

        $this->db->select('rm.*,s.dist_distributor,s.dist_distributorid, b.budg_budgetname,mt.maty_material,scf.loca_name schoolname,dtf.dept_depname fromdep,dtfp.dept_depname fromdepparent');
        $this->db->from('recm_receivedmaster rm');
        $this->db->join('dist_distributors s', 's.dist_distributorid = rm.recm_supplierid', 'LEFT');
        $this->db->join('budg_budgets b', 'b.budg_budgetid = rm.recm_budgetid', 'LEFT');
        // $this->db->where('recm_purchaseordermasterid !=',0);
    ...
    public function get_receive_against_order_details_list_ku($cond = false)
    {
        $get = $_GET;
        foreach ($get as $key => $value) {
            $get[$key] = strtolower(htmlspecialchars($get[$key], ENT_QUOTES));
        ...
        if (!empty($get['sSearch_6'])) {
            $this->db->where("recm_description like  '%" . $get['sSearch_6'] . "%'  ");
        }
        ...
        $this->db->select('rm.recm_receivedmasterid,rm.recm_purchaseorderno,rm.recm_fyear,recm_dstat,it.itli_itemname,it.itli_itemnamenp,s.dist_distributor, s.dist_distributorid, un.unit_unitname,rd.recd_purchasedqty,rd.recd_unitprice,(recd_purchasedqty*recd_unitprice) total,rd.recd_discountamt,rd.recd_vatamt,rd.recd_amount,rd.recd_locationid,rm.recm_receiveddatebs,rm.recm_receiveddatead,rm.recm_supbilldatebs, rm.recm_challanno,rd.recd_description,mt.maty_material,scf.loca_name schoolname,dtf.dept_depname fromdep,dtfp.dept_depname fromdepparent,rm.recm_receivedby,rd.recd_expdatead,rd.recd_expdatebs,recd_batchno');
    ...
    public function get_receive_against_order_details_list($cond = false)
    {
        ...
        $this->db->select('rm.recm_receivedmasterid,rm.recm_purchaseorderno,rm.recm_fyear,it.itli_itemname,recm_dstat,it.itli_itemnamenp,s.dist_distributor, s.dist_distributorid, un.unit_unitname,rd.recd_purchasedqty,rd.recd_unitprice,(recd_purchasedqty*recd_unitprice) total,rd.recd_discountamt,rd.recd_vatamt,rd.recd_amount,rd.recd_locationid,rm.recm_receiveddatebs,rm.recm_receiveddatead,rm.recm_supbilldatebs, rm.recm_challanno,rd.recd_expdatead,rd.recd_expdatebs,recd_batchno');
        $this->db->from('recd_receiveddetail rd');
        $this->db->join('recm_receivedmaster rm', 'rm.recm_receivedmasterid=rd.recd_receivedmasterid', "LEFT");
        $this->db->join('dist_distributors s', 's.dist_distributorid = rm.recm_supplierid', 'LEFT');
        $this->db->join('itli_itemslist it', 'it.itli_itemlistid=rd.recd_itemsid', 'LEFT');
                    ...
                    if ($insert_id) {
                        ...
                        if (!empty($itemsid)) {
                            foreach ($itemsid as $key => $val) :
                                ...
                                $recd_expiry_date = !empty($expiry_date[$key]) ? $expiry_date[$key] : '';
                                if (DEFAULT_DATEPICKER == 'NP') {
                                    $expirydatebs = $recd_expiry_date;
                                    $expirydatead = $this->general->NepToEngDateConv($recd_expiry_date);
                                } else {
                                    $expirydatead = $recd_expiry_date;
                                    $expirydatebs = $this->general->EngToNepDateConv($recd_expiry_date);
                                }
                        ...
                        if (!empty($master_insertid)) {
                            if (!empty($itemsid)) :
                                foreach ($itemsid as $key => $value) {
                                    ...
                                    $trde_expiry_date = !empty($expiry_date[$key]) ? $expiry_date[$key] : '';
                                    if (DEFAULT_DATEPICKER == 'NP') {
                                        $trde_expirydatebs = $trde_expiry_date;
                                        $trde_expirydatead = $this->general->NepToEngDateConv($trde_expiry_date);
                                    } else {
                                        $trde_expirydatead = $trde_expiry_date;
                                        $trde_expirydatebs = $this->general->EngToNepDateConv($trde_expiry_date);
                                    }
                                    $amount_total = !empty($amount[$key]) ? $amount[$key] : 0;
                                    ...
                                    $mattransDetailArray[] = array(
                                        ...
                                        'trde_controlno' => '',
                                        'trde_expdatebs' => $trde_expirydatebs,
                                        'trde_expdatead' => $trde_expirydatead,
                        ...
                        if (ORGANIZATION_NAME == 'KUKL') {
                            if (defined('RUN_API') && RUN_API == 'Y') {
                                if (defined('API_CALL') && API_CALL == 'KUKL') {
                                    $this->api_post_expense_amount($insert_id);
                                }
                            }
                        }
                    } // if insertid
    ...
    public function update_receive_data()
    {
        ...
        try {
            ...
            $batchno = $this->input->post('batchno');
            $supplierid = $this->input->post('supplierid');
            $recm_school = $this->input->post('recm_school');
            $recm_departmentid = $this->input->post('recm_departmentid');
            $recm_subdepartmentid = $this->input->post('recm_subdepartmentid');
            $departmentid = $recm_departmentid;
            if (!empty($recm_subdepartmentid)) {
                $departmentid = $recm_subdepartmentid;
            }
            ...
            $UpdatereceivedMasterArray = array(
                ...
                'recm_remarks' => $remarks,
                'recm_budgetid' => $bugetid,
                'recm_receivedby' => !empty($receivedby[1]) ? $receivedby[1] : '',
                'recm_receivedstaffid' => !empty($receivedby[0]) ? $receivedby[0] : ''
            );
            if (ORGANIZATION_NAME == 'KU') {
                $UpdatereceivedMasterArray['recm_school'] = $recm_school;
                ...
                foreach ($req_detail_list as $req_list) {
                    ...
                    if (!empty($req_detail)) {
                        ...
                        if (!empty($expense_data)) {
                            ...
                            $rem_block_amt = $expense_data->remaning_blocked_amount;

                            $total_remaning_amt = (float)($rem_block_amt - $recd_amount);
                            if ($total_remaning_amt > 0) {
                                //case for partially received 
                                $expense_status = 'PR';
                                $post_data = array(
                                    ...
                                    'Entry_By' => null,
                                    'Entry_Date' => null,
                                    "Remarks" => 'Partially Received',
                                    "insUp" => "UP",
                                    "Budget_id" => 0,
                                    "Amount" => $expense_data->block_amount,
                                    "Item_Description" => null,
                                    "Rem_Amount" => $total_remaning_amt,
                                    "Req_DateEn" => null,
                                    "Req_DateNp" => null,
                                    "Office_code" => 0,
                                    "Demand_No" => 0,
                            ...
                            if ($this->general->api_send_budget_demand_amount($post_data)) {
                                ...
                                $this->db->update('api_budgetexpense', array('remaning_blocked_amount' => $total_remaning_amt, 'status' => $expense_status));
                            }
                        }
                    }
                }
    ...
    public function save_receive_order_no_supplier()
    {
                    ...
                    if ($insert_id) {
                        ...
                        if (!empty($itemsid)) {
                            foreach ($itemsid as $key => $val) :
                                ...
                                $sup_id=!empty($supplierid[$key]) ? $supplierid[$key] : 0;
                                // echo $sup_id;
                                // die();
                                $sup_billno=!empty($bill_no[$key]) ? $bill_no[$key] : 0;
                                $bill_date =!empty($bill_date[$key]) ? $bill_date[$key] : 0;
                                $bugid=!empty($bugetid[$key]) ? $bugetid[$key] : 0;
                                
                                if (DEFAULT_DATEPICKER == 'NP') {
                                    $bill_datebs = $bill_date;
                                    $bill_datead = $this->general->NepToEngDateConv($bill_date);
                                } else {
                                    $bill_datead = $bill_date;
                                    $bill_datebs = $this->general->EngToNepDateConv($bill_date);
                                }
                                ...
                                $receivedDetailArray = array(
                                    'recd_receivedmasterid' => $insert_id,
                                    'recd_supplierid'=>$sup_id,
                                    'recd_supplier_datead'=>$bill_datead,
                                    'recd_supplier_datebs'=>$bill_datebs,
                                    'recd_supplier_billno'=> $sup_billno,
                                    'recd_budgetid'=>$bugid,
                                    'recd_itemsid' => !empty($itemsid[$key]) ? $itemsid[$key] : '',
                                    'recd_purchasedqty' => $purchased_qty,
                                    'recd_amount' => $amount_total,
                            ...
                            if (!empty($itemsid)) :
                                foreach ($itemsid as $key => $value) {
                                    ...
                                    $mattransDetailArray[] = array(
                                        ...
                                        'trde_unitprice' => !empty($actualrate) ? $actualrate : '0.00',
                                        'trde_selprice' => !empty($rate[$key]) ? $rate[$key] : '',
                                        'trde_supplierid' => $sup_id,
                                        'trde_mtmid' => $sup_id,
                                        'trde_supplierbillno' => $sup_billno,
                                        'trde_unitvolume' => 0,
                                        'trde_microunitid' => 0,
                                        'trde_totalvalue' => 0,
                                        'trde_description' => !empty($description[$key]) ? $description[$key] : '',
