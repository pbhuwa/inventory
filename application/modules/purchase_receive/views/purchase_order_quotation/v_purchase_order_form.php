...
<form method="post" id="FormReceiveOrderItem" action="<?php echo base_url('purchase_receive/purchase_order_quotation/save_order_item'); ?>" class="form-material form-horizontal form" data-reloadurl='<?php echo base_url('purchase_receive/purchase_order_quotation/form_purchase_order'); ?>'>
    <div id="orderData">
        <?php $this->load->view('purchase_order_quotation/v_order_form');?>
    </div>
                    ...
                    <tbody id="purchaseDataBody" class="requisitionOrder">
                        <?php if(!empty($orderdetails)) { 
                            ...
                            <tr class="orderrow" id="orderrow_<?php echo $key+1; ?>" data-id='<?php echo $key+1; ?>'>
                                <td>
                                    <input type="text" class="form-control" id="s_no_<?php echo $key+1; ?>" value="<?php echo $key+1; ?>" readonly/>
                                </td>
                                <td>
                                    <div class="dis_tab"> 
                                        <input type="text" class="form-control itemcode enterinput " id="itemcode_<?php echo $key+1; ?>" name="itemcode[]"  data-id='<?php echo $key+1; ?>' data-targetbtn='view' value="<?php echo $ord->itli_itemname; ?>" readonly />
                                        <input type="hidden" class="qude_itemsid" name="qude_itemsid[]" data-id='<?php echo $key+1; ?>' id="itemid_<?php echo $key+1; ?>" value="<?php echo $ord->pude_itemsid; ?>">
                                        <input type="hidden" value="<?php echo $ord->pude_itemsid; ?>" class="itemsid" name="itemsid[]" data-id='<?php echo $key+1; ?>' id="matdetailid_<?php echo $key+1; ?>">
                              ...
                              <!--   <?php// $tot = ($ord->purd_remqty) * ($ord->itli_salesrate); ?> -->
                                ...
                                <td>
                                    <input type="text" class="form-control multiInsert float calculateamt puit_qty" name="puit_qty[]" id="puit_qty_<?php echo $key+1; ?>" data-id="<?php echo $key+1; ?>" value="<?php echo $ord->pude_quantity; ?>">
                                </td>
                                <td>
                                ...
                                <td>
                                    <input type="text" name="discountpc[]" class="form-control float calculateamt discountpc"  placeholder="Discount Pc" id="discountpc_<?php echo $key+1; ?>" data-id="<?php echo $key+1; ?>" value="<?php echo !empty($ord->pude_discount)?$ord->pude_discount:'0' ; ?>">
                                </td>
                                <td>
                                    <input type="text" class="form-control float multiInsert calculateamt vatpc" name="puit_taxid[]" id="puit_taxid_<?php echo $key+1; ?>" value="<?php echo !empty($ord->pude_vat)?$ord->pude_vat:'0' ; ?>" data-id="<?php echo $key+1; ?>" >
                                </td>
                                <td>
                                ...
                                <td>
                                    <input type="text" class="form-control   multiInsert " name="description[]" id="description_<?php echo $key+1; ?>" data-id="<?php echo $key+1; ?>" placeholder="Remarks" value="<?php echo $ord->pude_remarks; ?>">
                                </td>
                                 <td>
...
<script>
  //calculate amount
    $(document).on('keyup change','.calculateamt', function(){
        ...
        parseTotalAmt = parseFloat(totalamt);
        // alert(totalamt);
        $('#puit_total_'+trid).val(parseTotalAmt.toFixed(2));
        ...
        stotal = stotal+extra-overall_discount;
...
<script type="text/javascript">
    ...
    $(document).on('keyup','#delivery_day',function(){
        var days=$(this).val();
        var action=base_url+'/purchase_receive/purchase_order_quotation/ajax_delivery_date_calculation';
        $.ajax({
            type: "POST",
            url: action,
            data:{days:days},
...
<script type="text/javascript">
    function getDetailList(masterid, main_form=false){
        if(main_form == 'main_form'){
            var submiturl = base_url+'purchase_receive/purchase_order_quotation/load_detail_list/new_detail_list';
            var displaydiv = '#displayDetailList'; 
        }else{
            var submiturl = base_url+'purchase_receive/purchase_order_quotation/load_detail_list';
...
<script type="text/javascript">
    $(document).off('change','.changeSupplier');
    $(document).on('change','.changeSupplier',function(){
        var supplierid = $('#supplierid').val();
        var fyear = $('#fiscalyear').val();
        var reqno = $('#requisitionNumber').val();

        if(reqno != ''){
            var displaydiv = '#displayDetailList'; 

            var submiturl = "<?php echo base_url('purchase_receive/purchase_order_quotation/load_supplier_rate');?>";

            var submitdata = {supplierid:supplierid, fyear:fyear, reqno:reqno};
            ...
            alert('Please choose requisition number first');

            setTimeout(function(){
                $('#supplierid').val(0).trigger('change.select2');
            },500);
        }
