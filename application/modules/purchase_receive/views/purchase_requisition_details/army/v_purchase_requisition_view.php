<?php 
$budget_head=$this->general->get_tbl_data('budg_budgetid,budg_budgetname','budg_budgets',array('budg_isactive'=>'Y'),'budg_budgetname','ASC');
// echo "<pre>";
    ...
    <div class="form-group row resp_xs">
        <div class="white-box pad-5">
            <div class="row">
                ...
                <?php $db_budget_headid=!empty($requistion_data[0]->pure_accheadid)?$requistion_data[0]->pure_accheadid:'0';
                $budget_head_name='';
                    if(!empty($db_budget_headid)){
                        $budget_head_result=$this->general->get_tbl_data('budg_budgetname','budg_budgets',array('budg_budgetid'=>$db_budget_headid),'budg_budgetname','ASC');
                        if(!empty($budget_head_result)){
                            $budget_head_name=!empty($budget_head_result[0]->budg_budgetname)?$budget_head_result[0]->budg_budgetname:'';
                        }
                    }
                ...
                <div class="col-sm-4">
                    ...
                    <?php echo !empty($budget_head_name)?$budget_head_name:'-'; ?>
                </div>
            ...
            <table style="width:100%;" class="table dataTable con_ttl dt_alt purs_table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo sprintf('%g',$value->purd_qty) ?></td>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-8">
                            ...
                            ?>
                                ...
                                <div class="col-sm-6 div_acc_bugethead" style="display:none;">
                                    <label>Acc. Head</label>
                                    <select name="acc_bugetheadid" class="form-control select2" id="acc_bugetheadid">
                                        <option value="0">--Select--</option>
                                        <?php 
                                            if(!empty($budget_head)): 
                                                foreach($budget_head as $bh):
                                            ?>
                                            <option value="<?php echo $bh->budg_budgetid ?>" <?php if($db_budget_headid==$bh->budg_budgetid): echo "selected=selected"; endif; ?>><?php echo $bh->budg_budgetname ?></option>
                                        <?php 
                                                endforeach;
                                            endif; ?>
                                    </select>
...
<script> 
    ...
    $(document).on('click','.cancel',function(){
        ...
        if(status=='Y'){
            $('.div_acc_bugethead').show();
        }else{
            $('.div_acc_bugethead').hide();
        }
    })
