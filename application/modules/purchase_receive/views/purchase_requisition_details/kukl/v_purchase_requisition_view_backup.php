...
<div class="form-group white-box pad-5">
 ...
 <div class="btn-group pull-left">
    <?php  
      ...
      if(in_array($this->usergroup, $view_order_approval_button_group)):
        if($requistion_data[0]->pure_isapproved=='S'){ ?>
        <a href="javascript:void(0)" class="btn btn-primary btnPurchaseApproval" data-val="M" style="margin-right:5px;">Approve</a>
    ...
    <?php   
      ...
      if(in_array($this->usergroup, $view_proc_supervisor_button_group)):
        if($requistion_data[0]->pure_isapproved=='M'){ ?>
        <a href="javascript:void(0)" class="btn btn-primary btnPurchaseApproval" data-val="D" style="margin-right:5px;">Approve</a>
    ...
    <?php   
      ...
      if(in_array($this->usergroup, $view_proc_head_button_group)):
        if($requistion_data[0]->pure_isapproved=='D'){ ?>
 ...
 <!--       <?php   if($requistion_data[0]->pure_isapproved !='C'){ ?>
    <button class="btn btn-success PrintThisNow ReprintThis pull-right" id="btnPrintNowBtn" data-print="print" data-actionurl="<?php echo base_url('/purchase_receive/purchase_requisition/purchase_requisition_reprint') ?>" data-viewdiv="FormDiv_Reprint" data-id="<?php echo !empty($requistion_data[0]->pure_purchasereqid)?$requistion_data[0]->pure_purchasereqid:''; ?>"><?php echo $this->lang->line('print'); ?></button>
    <?php } ?> -->
</div>
...
<div class="btn-group pull-left">
    <?php
        if(in_array($this->usergroup, $view_order_button_group)):
            if($requistion_data[0]->pure_isapproved=='Y'){ ?>
                <hr/>
                <strong>Select Purchase Type: </strong><br/>
                <input type="radio" name="select_purchase_type" id="type_order" class="select_purchase_type" data-pur_type="type_order" />Purchase Order &nbsp;&nbsp;&nbsp;
                <input type="radio" name="select_purchase_type" id="type_tender" class="select_purchase_type" data-pur_type="type_tender" />Tender &nbsp;&nbsp;&nbsp;
                <input type="radio" name="select_purchase_type" id="type_direct" class="select_purchase_type" data-pur_type="type_direct" />Direct Purchase


                <div class="div_order" style="display: none;">
                ...
                <div class="div_tender" style="display: none;">
                    <button style="margin-right: 5px;" class="btn btn-success btnredirect" data-print="print" data-viewurl="<?php echo base_url('/purchase_receive/tender') ?>"  data-id="<?php echo !empty($requistion_data[0]->pure_reqno)?$requistion_data[0]->pure_reqno:''; ?>">
                        <?php echo $this->lang->line('tender'); ?>
                    </button>
                </div>

                <div class="div_direct" style="display: none;">
                    ...
                    <?php
                        if(in_array($this->usergroup, $show_estimate_amt_group)):
                            ...
                            <td>
                                ...
                                ?>
                                <input type="text" value="<?php echo !empty($odr->purd_estimatecost)?$odr->purd_estimatecost:0; ?>" class="form-control estimate_cost arrow_keypress" data-fieldid="estimate_cost" id="estimate_cost_<?php echo $key+1; ?>" data-id='<?php echo $key+1; ?>' <?php echo $readonly; ?>/>
                            </td>
                            <?php
                        endif;
                    ?>
