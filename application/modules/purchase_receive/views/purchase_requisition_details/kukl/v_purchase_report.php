...
$centralrequest = !empty($purchase_requisition[0]->rema_centralrequest)?$purchase_requisition[0]->rema_centralrequest:'';
$proceedissue = !empty($purchase_requisition[0]->rema_proceedissue)?$purchase_requisition[0]->rema_proceedissue:'';
$proceedpurchase = !empty($purchase_requisition[0]->rema_proceedpurchase)?$purchase_requisition[0]->rema_proceedpurchase:'';
...
<div class="jo_form organizationInfo">
   <div class="headerWrapper">
      ...
      <table class="jo_tbl_head" width="100%" style="border-collapse: collapse;">
         ...
         <tr>
            <td width="80%">
               <span style="font-size: 12px; margin-bottom: 5px;" class="<?php echo FONT_CLASS; ?>"> श्री </span> <span class="borderbottom">
                  <?php
                  if($purchase_requisition){
                     echo !empty($purchase_requisition[0]->rema_reqby)?$purchase_requisition[0]->rema_reqby:''; 
                  }else{
            ...
            <td width="10%" style="text-align: center;padding-right: 70px;" class="text-center">
               ...
               <span class="borderbottom"> 
                  <?php 
                     echo !empty($purchase_requisition[0]->rema_reqno)?$purchase_requisition[0]->rema_reqno:''; ?> 
                  <?php }else{ ?>
                  <span class="borderbottom">
   ...
   <div class="tableWrapper">
               ...
               if($purchase_requisition){
                  ...
                  if(!empty($account_action_log)):
                     foreach($account_action_log as $log):
                        $account_comment .= $log->usma_fullname.'('.$log->aclo_actiondatebs.') -'.$log->aclo_comment.',';
                     endforeach;
                  endif;
                  ...
                  foreach($purchase_requisition as $key => $stock){
                     $cur_qty=!empty($stock->purd_qty)?$stock->purd_qty:0;
                     ...
                     $rate = !empty($stock->purd_estimatecost)?$stock->purd_estimatecost:0;
                     // $totamt = $cur_qty * $rate;
                     $totamt = !empty($stock->purd_estimatetotal)?$stock->purd_estimatetotal:0;
                     $fullremarks=  !empty($purchase_requisition[0]->rema_remarks)?$purchase_requisition[0]->rema_remarks:'';
               ?>
            <tr>
               <td class="td_cell">
                  <?php echo $key+1; ?>
               ...
               <td style="height: 100%; clear: both;overflow: hidden;" class="td_cell" rowspan="<?php echo $count_items+20; ?>">
                  <div style="writing-mode: vertical-rl;width: 100%;height: 100%; font-size:10px;">
                     ...
                     ?> 
                     <?php echo !empty($account_comment)?$account_comment:''; ?>
                     <br/>
                  </div>
               </td>
               <?php endif; ?> 
      ...
      <tfoot>
         <tr>
            <?php
               $grandtotal = !empty($purchase_requisition[0]->pure_estimateamt)?$purchase_requisition[0]->pure_estimateamt:'';
            ?>
...
<div class="footerWrapper">
   <table class="jo_footer" style="padding-top: 10px;padding-bottom: 10px;border: 0px solid #000;border-top: 0px;page-break-inside: avoid;">
      ...
      <td style="padding-top: 10px;">
         ...
         $demander_date = $purchase_requisition[0]->rema_reqdatebs;
         ...
         if(!empty($demander_fullname)):
            ?>
            <span class="signatureDashedLine spanborder signatureDateLine" >
              <?php 
...
<tr>
  ...
  <!--     <span class="signatureDashedLine spanborder">
      ...
      <span class="signatureDashedLine spanborder"></span>
...
<?php
   if(!empty($account_comment)):
?>
<div class="print_break_page" id="break_page" style='page-break-after:always'>
   <?php echo !empty($account_comment)?$account_comment:''; ?>
</div>
<?php
   endif;
