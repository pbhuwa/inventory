...
<!-- data-reloadurl="<? php // echo base_url('purchase_receive/receive_against_order/form_received_form'); 
                        ?>" -->
<form method="post" id="FormReceiveOrderItem" action="<?php echo base_url('purchase_receive/receive_against_order/save_received_items_no_supplier'); ?>" class="form-material form-horizontal form" data-reloadurl="<?php echo base_url('purchase_receive/receive_against_order/no_supplier/reload'); ?>" enctype="multipart/form-data" accept-charset="utf-8">
    <div id="orderData">
        ...
        <?php endif; ?>
            ...
            <div class="col-md-3 col-sm-3">
                ...
                <div class="dis_tab">
                    ...
                    &nbsp;
                    <a href="javascript:void(0)" data-id="0" data-displaydiv="Issue" data-viewurl="<?php echo base_url('purchase_receive/receive_against_order/load_order_list'); ?>" class="view table-cell width_30 btn btn-success" data-heading="Load Order list" id="orderload"><i class="fa fa-upload"></i></a>
                </div>
        ...
        <div class="form-group">
            ...
            <div class="col-md-3 col-sm-3">
                ...
                <input type="text" name="order_date" class="form-control  date" placeholder="Order Date" id="OrderDate" value="" readonly>
            ...
            <div class="col-md-3 col-sm-3">
                ...
                <input type="text" name="received_date" class="form-control required_field <?php echo DATEPICKER_CLASS; ?> date" placeholder="<?php echo $this->lang->line('received_date'); ?>" value="<?php echo DISPLAY_DATE; ?>" id="ReceivedDate">
                <span class="errmsg"></span>
            </div>
            ...
            <div class="col-md-3 col-sm-3" style="display: none" id="received_by_div">
                ...
                <select class="form-control select2" name="recm_receivedby">
                    ...
                    if ($this->location_ismain == 'Y' && in_array($this->usergroup, $this->show_location_group)) {
                        ...
                        $staff_cond = array(
                            'stin_jobstatus' => 'Y',
                            //'stin_locationid' => $this->locationid
                        );
                    }
                    $staff_info = $this->general->get_tbl_data('*', 'stin_staffinfo', $staff_cond);
                    if (!empty($staff_info)) :
                ...
                <table style="width:100%;" class="table purs_table dataTable">
                    <thead>
                        <tr>
                            ...
                            <th width="4%"><?php echo $this->lang->line('unit'); ?></th>
                            <th width="4%"><?php echo $this->lang->line('odr_qty'); ?></th>
                            <th width="4%"><?php echo $this->lang->line('qty'); ?></th>
                            <th width="5%"><?php echo $this->lang->line('rate'); ?></th>
                            <th width="4%"><?php echo $this->lang->line('cc'); ?></th>
                            <th width="4%"><?php echo $this->lang->line('dis'); ?></th>
                            <th width="4%"><?php echo $this->lang->line('vat'); ?><input type="text" class="form-control allvat" style="width:35px;height:20px" value="13"></th>
                            <th width="4%"><?php echo $this->lang->line('amount'); ?></th>
                            <?php
                            if (defined('SHOW_EXPIRY_ON_RECEIVE')) :
                                if (SHOW_EXPIRY_ON_RECEIVE == 'Y') :
                            ?>
                            ...
                            <th width="8%"><?php echo $this->lang->line('description'); ?></th>
                            <th width="8%">Budget<br>
                                <select class="table-cell form-control select2" id="budgetCheckall">
                            ...
                            <th width="12%">Supplier<br>
                            <select  class="form-control select2" readonly="readonly" id="supplierid_checkall">
                                <option value="">---select---</option>
                                <?php
                                if ($supplier_all) :
                            ...
                            <th width="5%">Bill no<br>
                                <input type="text" class="form-control" id="inp_billnumber">
                            </th>
                            <th width="6%">Bill Date <br>
                                 <input type="text" class="form-control <?php echo DATEPICKER_CLASS; ?>" value="<?php echo DISPLAY_DATE; ?>" id="inp_billdate">
                            </th>
                            <th width="2%"><?php echo $this->lang->line('action'); ?></th>
                        </tr>
                    </thead>
    ...
    <div class="form-group">
        <div class="col-md-12">
            <button type="submit" class="btn btn-info  save" data-operation='<?php echo !empty($order_item_details) ? 'update' : 'save' ?>' id="btnSubmit"><?php echo !empty($order_item_details) ? '' : 'Save' ?></button>
            <button type="submit" class="btn btn-info savePrint" data-operation='<?php echo !empty($order_item_details) ? 'update' : 'save ' ?>' id="btnSubmit" data-print="print"><?php echo !empty($order_item_details) ? 'Update' : 'Save & Print' ?></button>
        </div>
        <div class="col-sm-12">
...
<script type="text/javascript">
    ...
    $(document).on('click', '#btnSearchOrderno', function() {
        ...
        var submiturl = base_url + 'purchase_receive/receive_against_order/orderlist_by_order_no_no_supplier';
        ...
        function onSuccess(jsons) {
            ...
            if (orderdata) {
                ...
                var pdepid = orderdata.parentdepid;
                var schoolid = orderdata.schoolid;
                var subdepid = orderdata.depid;
                // console.log(pdepid);
                // console.log(subdepid);
                ...
                $('#locationid').val(schoolid).trigger('change');
                setTimeout(function() {
                    if (pdepid == null || pdepid == '') {
                        $('#rema_reqfromdepid').val(subdepid);
                    } else {
                        $('#rema_reqfromdepid').val(pdepid).trigger('change');
                        setTimeout(function() {
                            $('#subdepid').val(subdepid);
                        }, 500);
...
<script type="text/javascript">
    $(document).off('change','#budgetCheckall');
    $(document).on('change','#budgetCheckall',function(e){
        var cur_budget_val=$(this).val();
        if(cur_budget_val){
            // alert(cur_budget_val);
            // return false;

            $(".budgetopt").select2("val", cur_budget_val);
        }
    });

    $(document).off('change','#supplierid_checkall');
    $(document).on('change','#supplierid_checkall',function(e){
        var cur_supplierid_val=$(this).val();
        if(cur_supplierid_val){
            // alert(cur_supplierid_val);
            // return false;

            $(".supplieropt").select2("val", cur_supplierid_val);
        }
    });

$(document).off('keyup','#inp_billnumber');
$(document).on('keyup','#inp_billnumber',function(e){
        var cur_input_billno=$(this).val();
        // console.log(cur_input_billno);
        if(cur_input_billno){
            // alert(cur_input_billno);
            // return false;

        $('.bill_no_txt').each(function( key, value) {
          $(this).val(cur_input_billno);
      })
    } 
});

$(document).off('change keyup','#inp_billdate');
$(document).on('change keyup','#inp_billdate',function(e){
    var cur_bill_date=$('#inp_billdate').val();
    
    $('.bill_date_txt').each(function( key, value) {
          $(this).val(cur_bill_date);
      })
});
