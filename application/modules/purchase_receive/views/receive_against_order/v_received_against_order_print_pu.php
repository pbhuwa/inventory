				...
				if ($req_detail_list) {
					...
					foreach ($req_detail_list as $key => $direct) { ?>
						<tr style="border-bottom: 1px solid #212121;">
							...
							<td class="td_cell" style="text-overflow: ellipsis;overflow: hidden; overflow-wrap: break-word;text-align: right;">
								<?php
								$unit_vat = $direct->recd_unitprice * ($direct->recd_vatpc/100);
							...
							<td class="td_cell" style="text-align: right;">
								<?php
								// $total_wo_vat = ($direct->recd_unitprice * $direct->recd_purchasedqty) * (1+$direct->recd_vatpc/100);
								$total_wo_vat = ($direct->recd_unitprice * $direct->recd_purchasedqty);
								$sum_wo_vat +=$total_wo_vat;
								$unit_price_with_vat = $direct->recd_unitprice * (1+$direct->recd_vatpc/100);
								//echo number_format($total_wo_vat, 2);
								echo number_format($unit_price_with_vat, 2);
								?>
							</td>
							<td class="td_cell"></td>
							<td class="td_cell" style="text-overflow: ellipsis;overflow: hidden; overflow-wrap: break-word; text-align: right;">
