...
<form method="post" id="FormReceiveOrderItem" action="<?php echo base_url('purchase_receive/purchase_order/save_order_item'); ?>" class="form-material form-horizontal form" data-reloadurl='<?php echo base_url('purchase_receive/purchase_order/form_purchase_order'); ?>'>
    <div id="orderData">
        <?php $this->load->view('purchase_order/v_order_form');?>
    </div>
                ...
                <div class="col-sm-6 pull-right">
                    <fieldset class="pull-right bordered mh_init">
                        <ul>
                            ...
                            <li>
                                <label>
                                    <?php echo !empty($this->lang->line('overall_vatamount'))?$this->lang->line('overall_vatamount'):'Overall Vat'; ?>
                                </label>
                                <?php $overall_vatamount=!empty($order_details[0]->puor_vatamount)?$order_details[0]->puor_vatamount:'0'; ?>
                                <input type="text" class="form-control float" name="taxtamount" id="overall_vatamount" value="<?php echo !empty($overall_vatamount)?$overall_vatamount:'0'; ?>" data-discount="amt" / >
                            </li>
                            <li>
...
<script>
    ...
    $(document).on('keyup change','.calculateamt', function(){
        ...
        var overall_vatamount = parseFloat($('#overall_vatamount').val());
        ...
        if(isNaN(overall_vatamount)){
            overall_vatamount = 0;
        }else{
            overall_vatamount = overall_vatamount;
        }
        ...
        stotal = stotal+extra-overall_discount + overall_vatamount;
    ...
    //overall TAX
    $(document).off('keyup change','#overall_vatamount');
    $(document).on('keyup change','#overall_vatamount',function(){
        var discount = $('#overall_vatamount').val();
...
<!-- VAT JS -->
<script type="text/javascript">
    $(document).off('change keyup','.puit_taxid');
    $(document).on('change keyup','.puit_taxid',function(){
        
        var disArray = [];
        i = 0;
        total = 0;
        $('.puit_taxid').each(function(i){  
            field_tax = this.value;

            valid_field_tax = checkValidValue(field_tax);

            disArray[i++]=valid_field_tax;
            total+=parseFloat(valid_field_tax);
        });
        ...
        if(valid_total > 0){
            $('#overall_vatamount').prop("disabled", true);
            $('#taxtamount').prop("disabled", false);
            $('#overall_vatamount').val('0');
        }else{
            $('#taxtamount').prop("disabled", true);
            $('#overall_vatamount').prop("disabled", false);
             $('#taxtamount').val('0');
        }
    });
</script>