...
class Stock_expired_mdl extends CI_Model
{
    ...
    public function get_stock_expired_list($cond = false)
    {
        $get = $_GET;
    ...
    if(!empty($get['sSearch_1'])){
        ...
        if($frmDate &&  $toDate){
            ...
            {
              $this->db->where('mtd.trde_expdatebs >=', $frmDate);
              $this->db->where('mtd.trde_expdatebs <=', $toDate);
            }
            else
            {
              $this->db->where('mtd.trde_expdatead >=', $frmDate);
              $this->db->where('mtd.trde_expdatead <=', $toDate);
            }
        }
