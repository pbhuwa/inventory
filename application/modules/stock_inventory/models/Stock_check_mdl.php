<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
class Stock_check_mdl extends CI_Model 
{
	public function __construct() 
	{
		parent::__construct();

        $this->curtime = $this->general->get_currenttime();
        $this->userid = $this->session->userdata(USER_ID);
        $this->mac = $this->general->get_Mac_Address();
        $this->ip = $this->general->get_real_ipaddr();
           $this->locationid=$this->session->userdata(LOCATION_ID);
    $this->location_ismain=$this->session->userdata(ISMAIN_LOCATION);
	}
	public $validate_settings_distributors = array(               
        array('field' => 'dist_distributor', 'label' => 'Distributor Name', 'rules' => 'trim|required|xss_clean'),
        array('field' => 'dist_phone1', 'label' => 'Phone 1', 'rules' => 'trim|required|xss_clean'),
        array('field' => 'dist_email', 'label' => 'Email', 'rules' => 'trim|valid_email|xss_clean'),
        array('field' => 'dist_repemail', 'label' => 'Sales Resp. Email', 'rules' => 'trim|valid_email|xss_clean')
	);

	public function get_stock_check_list($cond = false)
	{
		$get = $_GET;
      	foreach ($get as $key => $value) {
        	$get[$key] = strtolower(htmlspecialchars($get[$key], ENT_QUOTES));
      	}

        if(!empty($get['sSearch_1'])){
            $this->db->where("itli_itemcode like  '%".$get['sSearch_1']."%'  ");
        }

        if(!empty($get['sSearch_2'])){
            $this->db->where("itli_itemname like  '%".$get['sSearch_2']."%' OR itli_itemnamenp like  '%".$get['sSearch_2']."%'  ");
        }
        
        if($cond) {
            $this->db->where($cond);
        }



        $resltrpt=$this->db->select("COUNT(*) as cnt")
                    ->from('itli_itemslist il')
                    ->get()
                    ->row();

      	// echo $this->db->last_query();die(); 
      	$totalfilteredrecs=$resltrpt->cnt; 

        $order_by = 'itli_itemname';
        $order = 'asc';
    
      	$where='';
      	if($this->input->get('iSortCol_0')==1)
        	$order_by = 'itli_itemcode';
      	else if($this->input->get('iSortCol_0')==2)
       		$order_by = 'itli_itemname';
        
      	$totalrecs='';
      	$limit = 15;
      	$offset = 1;
      	$get = $_GET;
 
	    foreach ($get as $key => $value) {
	        $get[$key] = strtolower(htmlspecialchars($get[$key], ENT_QUOTES));
	    }
      
        if(!empty($_GET["iDisplayLength"])){
           $limit = $_GET['iDisplayLength'];
           $offset = $_GET["iDisplayStart"];
        }

        if(!empty($get['sSearch_1'])){
            $this->db->where("itli_itemcode like  '%".$get['sSearch_1']."%'  ");
        }

        if(!empty($get['sSearch_2'])){
            $this->db->where("itli_itemname like  '%".$get['sSearch_2']."%' OR itli_itemnamenp like  '%".$get['sSearch_2']."%'  ");
        }
       
        if($cond) {
          $this->db->where($cond);
        }

        $this->db->select('il.itli_itemlistid as itemid, il.itli_itemcode as itemcode, il.itli_itemname as itemname, il.itli_itemnamenp as itemnamenp,il.itli_locationid');
        $this->db->from('itli_itemslist il');
        
         $this->db->order_by($order_by,$order);
      
        if($limit && $limit>0)
        {  
            $this->db->limit($limit);
        }
        if($offset)
        {
            $this->db->offset($offset);
        }
      
        $nquery=$this->db->get();

        // echo $this->db->last_query();
        // die();
        $num_row=$nquery->num_rows();
        if (!empty($_GET['iDisplayLength']) &&  !empty($nquery) && is_array($nquery) && count($nquery) > 0 ) {
            $totalrecs = sizeof($nquery);
        }

       if($num_row>0){
          $ndata=$nquery->result();
          $ndata['totalrecs'] = $totalrecs;
          $ndata['totalfilteredrecs'] = $totalfilteredrecs;
        } 
        else
        {
            $ndata=array();
            $ndata['totalrecs'] = 0;
            $ndata['totalfilteredrecs'] = 0;
        }
	    // echo $this->db->last_query();die();
	   return $ndata;
	}

    public function get_stock_check_by_id($srchcol = false){
        try{
          
            $this->db->select('mtm.trma_todepartmentid as todepartment, sum(mtd.trde_issueqty) as stockqty');
            $this->db->from('trma_transactionmain mtm');
            $this->db->join('trde_transactiondetail mtd','mtm.trma_trmaid = mtd.trde_trmaid');
            $this->db->where('mtm.trma_received','1');
            if($srchcol){
                $this->db->where($srchcol);
            }
            $this->db->group_by('mtm.trma_todepartmentid');
            $query = $this->db->get();

            if($query->num_rows() >0){
                $result = $query->result();
                return $result;
            }
        }catch(Exception $e){
            throw $e;
        }
    }
    
    public function get_stock_till_date_search($srchcol = false, $issummary = false)
    {
        try{
            $this->db->select('mt.maty_material, et.eqty_equipmenttype, SUM(cd.csde_transactionvalue) as intransaction,SUM(cd.csde_stockvalue) as instock, SUM(cd.csde_transactionvalue + cd.csde_stockvalue)  as totalstockval,mt.maty_locationid');
            $this->db->from('csde_closingstockdetail cd');
            $this->db->join('itli_itemslist il', 'il.itli_itemlistid = cd.csde_itemsid', "LEFT");
            $this->db->join('maty_materialtype mt','mt.maty_materialtypeid = il.itli_materialtypeid','LEFT');
            $this->db->join('eqty_equipmenttype et','et.eqty_equipmenttypeid = il.itli_typeid','LEFT');
            if($srchcol){
                $this->db->where($srchcol);
            }
            
            if($issummary == 'summary'){
                $this->db->group_by('mt.maty_material');
            }else{
                $this->db->group_by('et.eqty_equipmenttype');
            }
            $query = $this->db->get();

            // echo $this->db->last_query();
            // die();

            if($query->num_rows() >0){
                $result = $query->result();
                return $result;
            }
        }catch(Exception $e){
            throw $e;
        }
    }
    
    public function get_changedata($srchcol = false, $order=false)
    {
        try{
            $this->db->select('clsm_csmasterid,clsm_todatebs,clsm_fromdatebs');
            $this->db->from('clsm_closingstockmaster');
            $this->db->where('clsm_complete','Y');
            if($srchcol){
                
                $this->db->where($srchcol);
            }
            if($order){
                
                $this->db->order_by($order);
            }
            $this->db->order_by('clsm_todatebs');
            $query = $this->db->get();

            if($query->num_rows() >0){
                $result = $query->result();
                return $result;
            }
        }catch(Exception $e){
            throw $e;
        }
    }
}