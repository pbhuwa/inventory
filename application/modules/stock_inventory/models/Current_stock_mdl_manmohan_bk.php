...
class Current_stock_mdl extends CI_Model 
{
    ...
    public function get_current_stock_lists($cond= false)
    {
        ...
        if(!empty($get['sSearch_5'])){
            $this->db->where("maty_material like  '%".$get['sSearch_5']."%'  ");
        }
        if(!empty($get['sSearch_6'])){
            $this->db->where("itli_maxlimit like  '%".$get['sSearch_6']."%'  ");
        }
        if(!empty($get['sSearch_7'])){
            $this->db->where("itli_reorderlevel like  '%".$get['sSearch_7']."%'  ");
        }
        if(!empty($get['sSearch_8'])){
            $this->db->where("trde_issueqty like  '%".$get['sSearch_8']."%'  ");
        }
        if(!empty($get['sSearch_9'])){
            $this->db->where("stockrmk like  '%".$get['sSearch_9']."%'  ");
        }
        if(!empty($get['sSearch_10'])){
            $this->db->where("loca_name like  '%".$get['sSearch_10']."%'  ");
        }
    ...
    public function get_current_stock_detail_lists($cond= false)
    {
        ...
        if(!empty($get['sSearch_6'])){
            $this->db->where("itli_reorderlevel like  '%".$get['sSearch_6']."%'  ");
        }
        if(!empty($get['sSearch_7'])){
            $this->db->where("totalstock like  '%".$get['sSearch_7']."%'  ");
        }
        if(!empty($get['sSearch_8'])){
            $this->db->where("stockrmk like  '%".$get['sSearch_8']."%'  ");
        }
        if(!empty($get['sSearch_9'])){
            if(DEFAULT_DATEPICKER == 'NP')
            {
                $this->db->where("trde_transactiondatebs like  '%".$get['sSearch_9']."%'  ");
            }else{
                $this->db->where("trde_transactiondatead like  '%".$get['sSearch_9']."%'  ");
            }
        }
         if(!empty($get['sSearch_10'])){
            $this->db->where("loca_name like  '%".$get['sSearch_10']."%'  ");
        ...
        if(!empty($get['sSearch_11'])){
            $this->db->where("trde_transactiontype like  '%".$get['sSearch_11']."%'  ");
        }
        // if($cond) {
        //     $this->db->where($cond);
        // }
        ...
        if(!empty($get['sSearch_5'])){
            $this->db->where("itli_maxlimit like  '%".$get['sSearch_5']."%'  ");
        }
        if(!empty($get['sSearch_6'])){
            $this->db->where("itli_reorderlevel like  '%".$get['sSearch_6']."%'  ");
        }
    ...
    public function get_location_stock_detail_lists($cond=false)
    {
        ...
        if(!empty($get['sSearch_6'])){
            $this->db->where("trde_issueqty like  '%".$get['sSearch_6']."%'  ");
        }
        
        if(!empty($get['sSearch_7'])){
            $this->db->where("trde_unitprice like  '%".$get['sSearch_7']."%'  ");
        }
  ...
  public function generate_temp_stock_location()
    ...
    public function get_distinct_location_from_store()
    {
        ...
        DISTINCT(lost_locationid) as locid , loc.loca_name FROM xw_lost_locationstock ls 
            LEFT JOIN xw_loca_location loc on loc.loca_locationid=ls.lost_locationid 
...
public function get_current_stock_detail($cat_id=false)
  {
    ...
    if($locid)
    {
        ...
        $prev_cond.=" AND trma_locationid =$locid";
    }
    if($storid)
    {
        $cond.=" AND trma_fromdepartmentid =$storid";
        $prev_cond.=" AND trma_fromdepartmentid =$storid";
    }
    if($catid)
    {
        $cond.= " AND eq.eqca_equipmentcategoryid =$catid";
        $prev_cond.= " AND eq.eqca_equipmentcategoryid =$catid";
    }
    if($cat_id)
    {
         $cond.= " AND il.itli_catid= $cat_id";
         $prev_cond.= " AND il.itli_catid= $cat_id";
    }
    else
    {
         $cond.=" AND il.itli_catid IS NULL ";
         $prev_cond.=" AND il.itli_catid IS NULL ";
    }
    if($fromdate &&  $todate){
            if(DEFAULT_DATEPICKER=='NP')
            {
               $cond.=" AND trma_transactiondatebs >= '$fromdate' AND trma_transactiondatebs <= '$todate'";
                $prev_cond .= "AND trma_transactiondatebs <= '$fromdate'";
            }
            else
            {
        ...
        if(defined('INVENTORY_VALUATION')){
            if(INVENTORY_VALUATION=='FIFO'){
                 $data= $this->db->query('SELECT eq.eqca_category,tm.trma_transactiondatebs,tm.trma_transactiondatead,il.itli_itemname,il.itli_itemnamenp,td.trde_itemsid,il.itli_itemcode,rd.recd_vatpc,SUM(td.trde_issueqty)trde_issueqty,td.trde_unitprice,(sum(td.trde_issueqty)*td.trde_unitprice) tamount from xw_trma_transactionmain tm LEFT JOIN xw_trde_transactiondetail td on td.trde_trmaid=tm.trma_trmaid
    ...
    eqca_category,
    trma_transactiondatebs,
    trma_transactiondatead,
    itli_itemname,
    itli_itemnamenp,
    trde_itemsid,
    itli_itemcode,
    recd_vatpc,
    SUM(cur_issueqty) + SUM(trde_issueqty) as trde_issueqty,
    SUM(cur_unitprice) + SUM(trde_unitprice) as trde_unitprice,
    SUM(cur_tamount) + SUM(tamount) as tamount
    FROM (
SELECT
    eq.eqca_category,
    tm.trma_transactiondatebs,
    tm.trma_transactiondatead,
    il.itli_itemname,
    il.itli_itemnamenp,
    td.trde_itemsid,
    il.itli_itemcode,
    ...
    0 as cur_unitprice,
    0 as cur_tamount,
    SUM( td.trde_issueqty ) trde_issueqty,
    SUM( td.trde_issueqty * td.trde_unitprice )/ SUM( td.trde_issueqty ) trde_unitprice,
    SUM( td.trde_issueqty )*(
    SUM( td.trde_issueqty * td.trde_unitprice )/ SUM( td.trde_issueqty )) tamount 
FROM
    xw_trma_transactionmain tm
...
WHERE
    ...
    il.itli_itemcode,
    rd.recd_vatpc,
    SUM( td.trde_issueqty ) cur_issueqty,
    SUM( td.trde_issueqty * td.trde_unitprice )/ SUM( td.trde_issueqty ) cur_unitprice,
    SUM( td.trde_issueqty )*(
    SUM( td.trde_issueqty * td.trde_unitprice )/ SUM( td.trde_issueqty )) cur_tamount,
         0 AS trde_issueqty,
         0 AS trde_unitprice,
         0 AS tamount
FROM
...
GROUP BY
    td.trde_itemsid,
    il.itli_itemname) X GROUP BY X.trde_itemsid, X.itli_itemname")->result();
