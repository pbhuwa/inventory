...
class Material_mdl extends CI_Model 

{
...

	{
		...
		$this->general->save_log($this->table,'maty_materialtype',$id,$postdata,'Update');

		$this->db->update($this->table,$postdata,array('maty_materialtypeid'=>$id));
	...
	public function get_all_materials($srchcol=false)

	{

		$this->db->select('mt.*');

		 $this->db->from('maty_materialtype mt');
	...
	public function get_all_material_list($srchcol=false)
...

	{
...

		{
...

			{
				 ...
				 $this->general->save_log($this->table,'maty_materialtypeid',$id,$postdata,'Update');
...

		{
...

			{
				...
				$this->db->insert('maty_materialtype',$postdata);
...

	{
...

		{
			 ...
			 $this->general->save_log($this->table,'maty_materialtypeid',$id,$postdata=array(),'Delete');

			$this->db->delete($this->table,array('maty_materialtypeid'=>$id));
...

	{
		...
		$get = $_GET;
     	...
     	if(!empty($get['sSearch_0'])){
            ...
            $this->db->where("lower(maty_materialtypeid) like  '%".$get['sSearch_0']."%'  ");
        ...
        if(!empty($get['sSearch_1'])){
            ...
            $this->db->where("lower(maty_material) like  '%".$get['sSearch_1']."%'  ");
		...
		$this->db->from('maty_materialtype');
...
public function save_equipment_cat()
	...
	public function check_exist_material_for_other($materialtype = false, $id = false){
...

		{
			...
			$query = $this->db->get_where($this->table,array('maty_material'=>$materialtype,'maty_materialtypeid !='=>$id));
...

		{
			...
			$query = $this->db->get_where($this->table,array('maty_material'=>$s));
