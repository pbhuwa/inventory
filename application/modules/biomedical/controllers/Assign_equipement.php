...
class Assign_equipement extends CI_Controller
{
	...
	public function index()
    {
		...
		$this->data['dep_information']=$this->department_mdl->get_all_department(false,false,false,'dept_depname','ASC');
		// $this->template
		// 	->set_layout('general')
		// 	->enable_parser(FALSE)
		// 	->title($this->page_title)
		// 	->build('assign_equipement/v_assign_equipement', $this->data);
	
		$this->data['tab_type']='equipment_assignment';
		$this->template
			->set_layout('general')
			->enable_parser(FALSE)
			->title($this->page_title)
			->build('assign_equipement/v_assign_equip_common_tab', $this->data);
	}

	public function assign_list_equipment($type=false)
    {
		$this->data['editurl']=base_url().'biomedical/equipments/editdeequipment';
		$this->data['deleteurl']=base_url().'biomedical/equipments/deletedeequipment';
		$this->data['listurl']=base_url().'biomedical/dep_change/v_dep_change_log';
		
		$this->data['breadcrumb']='Equipments/Assigned List';
		$seo_data='';
		if($seo_data)
		{
			//set SEO data
		...
		$this->data['tab_type']='equipment_assign_list';
	...
	public function assign_equipement_list($result=false,$orgid=false)
	{
		if(MODULES_VIEW=='N')
			{
			 	$array['equipkey'] = '';
		...
		if($useraccess == 'B')
		{
			...
			$this->data = $this->assign_equipement_mdl->get_assign_equipment($srchcol);
		}else{
			$cond = array('eqas_orgid'=>$org_id);
			$this->data = $this->assign_equipement_mdl->get_assign_equipment($cond);
		}
		//print_r($org_id);die;
		if($result == 'cur') {
			$cond=array('eqas_assigndatead'=>date("Y/m/d"), 'eqas_orgid'=>$org_id);
			$this->data = $this->assign_equipement_mdl->amc_summary($cond);
	    ...
	    unset($this->data["totalfilteredrecs"]);
	  	unset($this->data["totalrecs"]);
		  	...
		  	foreach($this->data as $row)
		    {
			    ...
			    $array[$i]["equipkey"] = '<a href="javascript:void(0)" class="patlist" data-equipid='.$row->bmin_equipmentkey.'>'.$row->bmin_equipmentkey.'</a>';
			    ...
			    $array[$i]["roomname"] = $row->rode_roomname;
			    $array[$i]["action"]='';
	...
	public function handover()
	{
		...
		$this->data['breadcrumb']='Equipments/Handover ';
		// $this->template
		// 	->set_layout('general')
		// 	->enable_parser(FALSE)
		// 	->title($this->page_title)
		// 	->build('assign_equipement/v_handover', $this->data);


		$this->data['tab_type']='equipment_handover';
		$this->template
			->set_layout('general')
			->enable_parser(FALSE)
			->title($this->page_title)
	...
	public function get_equipment_detail()
	{
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		$tempform.=$this->load->view('assign_equipement/v_department_staff',$this->data,true);
		if(!empty($tempform))
			{
	...
	public function equipment_assign_multiple()
	{
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		$this->load->Model('staff_info_mdl');
		$this->data=array();
		...
		$this->data['equipment_list']=$this->bio_medical_mdl->get_biomedical_inventory("bmin_equipid IN ($ids)");
	...
	public function save_equipment_assign()
	{
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			 $trans = $this->assign_equipement_mdl->assign_equipment_save();
            if($trans)
            {
