<div class="row wb_form">
    <div class="col-sm-6">
        <div class="white-box">
            <h3 class="box-title">Equipment Setup </h3>
            <div id="FormDiv_biomedicalinventory" class="formdiv frm_bdy">
            <?php $this->load->view('bio_medical_inventory/v_bio_medical_inventoryform') ;?>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
  $(document).on('change','#bmin_equipmenttypeid',function(){
    var eqtypeid=$(this).val();
    var action=base_url+'biomedical/equipments/get_equipments_from_eqtypeid';
    // alert(depid);
     $.ajax({
          type: "POST",
          url: action,
        ...
        success: function(datas) 
          {
            ...
            var opteq='';
                ...
                {
                  opteq += '<option value='+k.eqli_equipmentlistid+'>'+k.eqli_description+'</option>';
                });
          $('.equipmentdesc').html(opteq);
          }
      });
  })
</script>