<table class="table flatTable tcTable compact_Table">
  ...
  <tbody>
    <?php if($amc_data_rec):
      foreach ($amc_data_rec as $kpmr => $rec):
     ?>
    <tr>
      <td>
        <?php echo $rec->amta_amcdatead;  ?>
      </td>
      <td>
        <?php echo $rec->amta_amcdatebs;  ?>
      </td>
      <td>
        <?php echo $rec->amta_remarks;  ?>
      </td>
      <td>
        <?php echo $rec->amta_postdatead;  ?>
      </td>
       <td>
        <?php echo $rec->amta_postdatebs;  ?>
      </td>
       <td>
        <?php echo $rec->amta_postip;  ?>
      </td>
...
<form method="post" id="formIsPmCompleted" action="<?php echo base_url('biomedical/amc_completed/save_amc_completed'); ?>" class="form-material form-horizontal form" data-reloadurl='<?php echo base_url('biomedical/amc_data'); ?>'>
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-md-2">
                       <label for="example-text">AMC:
                          ...
                          <select name="amta_amc" class="form-control" id="amc" >
                      ...
                      <div class="col-md-3">
                        <label>AMC Complete Date</label>
                          <input type="text" name="amc_amccompleteddate" class="form-control date <?php echo DATEPICKER_CLASS; ?>" value="<?php echo DISPLAY_DATE; ?>" id="pm_completeddate">
                  ...
                  <div class="clearfix"></div>
                  <button type="submit" class="btn btn-info waves-effect waves-light m-r-10 savelist" data-isdismiss="Y" data-isrefresh="Y" data-operation='<?php echo !empty($pm_completed_data)?'update':'save' ?>'><?php echo !empty($pm_completed_data)?'Update':'Save' ?></button>
