...
<div class="jo_form organizationInfo">
    <div class="headerWrapper"  >
        <div class="ku_print_header">
             <?php 
$header['report_title'] = 'Asset Transfer';
...
<div style="margin-top: -30px;">
              ...
              <div class="text-right">Transfer Date : <?php echo !empty($transfer_data[0]->astm_transferdatebs)?$transfer_data[0]->astm_transferdatebs:''; ?></div> 
        </div>
    </div>
<div class="ku_details"  >
        <div class="ku_details_individual" >
                <h6 style="grid-template-columns: 30% 3% 67%">Fiscal Year 
                        <span>:</span>
                    <span class="ku_value">
                       <?php echo !empty($transfer_data[0]->astm_fiscalyrs)?$transfer_data[0]->astm_fiscalyrs:''; ?>  
                    </span>
                </h6>
                <h6 >Transfer Code<span>:</span>
                    <span class="ku_value">
                <?php echo !empty($transfer_data[0]->astm_transferno)?$transfer_data[0]->astm_transferno:''; ?>
                    
                    </span>
                </h6>
                <h6 >Manual Code<span>:</span>
                    <span class="ku_value">
                <?php echo !empty($transfer_data[0]->astm_manualno)?$transfer_data[0]->astm_manualno:''; ?>
        ...
        <div class="ku_details_individual.full" >
              ...
              $school_id=!empty($transfer_data[0]->astm_fromschoolid)?$transfer_data[0]->astm_fromschoolid:'';
 ...
 <?php 
            ...
            $reqdepartment=!empty($transfer_data[0]->astm_from)?$transfer_data[0]->astm_from:'';
        ...
        <div class="ku_details_individual.full">
                <h6 >Transfer To <span>:</span>
                    <span class="ku_value">
            <?php
            $school_id=!empty($transfer_data[0]->astm_toschoolid)?$transfer_data[0]->astm_toschoolid:'';
            $school_result=$this->general->get_tbl_data('*','loca_location',array('loca_locationid'=>$school_id),'loca_name','ASC'); 
            if(!empty($school_result)){
            echo !empty($school_result[0]->loca_name)?$school_result[0]->loca_name:'';
            echo "-";
        ...
        ?>
 <?php 
            $reqdepartment=!empty($transfer_data[0]->astm_to)?$transfer_data[0]->astm_to:'';
            $check_parentid=$this->general->get_tbl_data('*','dept_department',array('dept_depid'=>$reqdepartment),'dept_depname','ASC');
            $dep_parent_dep_name='';
            $sub_depname='';
            if(!empty($check_parentid)){
        ...
        ?>
            ...
            <div class="ku_details_individual.full" >
                <h6>Received By<span>:</span>
                    <span class="ku_value">
                        <?php 
                         echo !empty($transfer_data[0]->astm_receivedby)?$transfer_data[0]->astm_receivedby:''; 
                        ?>
                    </span>
                </h6>
            </div>
    ...
    <table class="ku_table table table-borderless " width="100%" style="margin:1.5rem 0">
        ...
        <tbody>
            ...
            if(!empty($transfer_data)): 
                ...
                foreach ($transfer_data as $kwd => $tdata):
