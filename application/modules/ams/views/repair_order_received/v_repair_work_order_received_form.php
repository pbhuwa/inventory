...
<form method="post" id="FormReceiveOrderItem" action="<?php echo base_url('ams/repair_work_order_received/save_repair_order_received'); ?>" class="form-material form-horizontal form" data-reloadurl="<?php echo base_url('ams/repair_work_order_received/index/reload'); ?>" enctype="multipart/form-data" accept-charset="utf-8">
    <div id="orderData">
        <input type="hidden" name="id" value="" />
        <input type="hidden" name="workordermasterid" id="workordermasterid" />
        <div class="form-group">
            ...
            <div class="col-md-3 col-sm-3">
                ...
                <label for="example-text">Work Order No.<span class="required">*</span>:</label>
                <div class="dis_tab">
                    <input type="text" class="form-control required_field enterinput" name="orderno" value="" placeholder="Enter Work Order Number" id="orderno" data-targetbtn='btnSearchOrderno'>
                ...
                <table style="width:100%;" class="table purs_table dataTable">
                    <thead>
                        <tr>
                            ...
                            <th width="11%">Assets Code</th>
                            <th width="15%">Assets Desc</th>
                            
                             <th width="7%"><?php echo $this->lang->line('problem'); ?></th>
                           
                            <th width="8%"><?php echo $this->lang->line('rate'); ?></th>
                            <th width="8%"><?php echo $this->lang->line('cc'); ?></th>
                          
                            <th width="8%"><?php echo $this->lang->line('vat'); ?><input type="text" class="form-control allvat" style="width:35px;height:20px" value="13"></th>
...
<script type="text/javascript">
    ...
    $(document).on('click', '#btnSearchOrderno', function() {
        ...
        var submiturl = base_url + 'ams/repair_work_order_received/workorderlist_by_order_no';
        ...
        function onSuccess(jsons) {
            ...
            var orderdata = data.work_order_data;
            // console.log(data);

            var defaultdatepicker = '<?php echo DEFAULT_DATEPICKER ?>';

            if (orderdata) {
                var orderdatead = orderdata[0].rewm_repairorderdatead;
                var orderdatebs = orderdata[0].rewm_repairorderdatebs;
                var supplierid = orderdata[0].rewm_supplierid;
                var workordermasterid = orderdata[0].rewm_repairordermasterid;
                ...
                $('#workordermasterid').val(workordermasterid);
                ...
                if (main_form == 'main_form') {
                    if (data.status == 'success') {
                        if (data.isempty == 'empty') {
                            ...
                            // workOrderData = data.work_order_data;
                            // console.log(workOrderData);
                            // $('#supplierid').val(workOrderData.rewm_supplierid);
                            // $('#OrderDate').val(workOrderData.rewm_repairorderdatebs);
...
<script type="text/javascript">
    ...
    $(document).on('change', '.chooseMatType,#fiscalyear', function() {
        ...
        var fyear = $('#fiscalyear').val();
        var purreqid = $('#workordermasterid').val();

        $('#orderload').attr('data-id', matType);
        $('#orderload').data('id', matType);
        if (purreqid) {
            $('#workordermasterid').val('');
            $('.orderrow').remove();
