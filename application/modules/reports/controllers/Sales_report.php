...
class Sales_report extends CI_Controller
{
  function __construct()
  {
    ...
    parent::__construct();
    $this->load->Model('sales_report_mdl');
  ...
  public function index($analysis_type=false)
  {
    ...
    $this->template
      ...
      ->build('sales_report/v_report_common', $this->data);
  }
 ...
 public function sales_report_details()
  {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $template = $this->sales_report_data();
  ...
  public function sales_report_data(){
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $this->data['excel_url'] = "reports/sales_report/generate_sales_report_excel";
            $this->data['pdf_url'] = "reports/sales_report/generate_sales_report_pdf";

            $this->data['report_title'] ="Sales Report";
            ...
            $this->data['get_sales_report'] = $this->sales_report_mdl->get_sales_report('sales_report');
           ...
           if($this->location_ismain=='Y'){
            ...
            if(!empty($this->data['get_sales_report'])){
                $template=$this->load->view('sales_report/v_sales_report_list', $this->data,true);
            }else{
     ...
     public function generate_sales_report_excel()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            header("Content-Type: application/xls");    
            header("Content-Disposition: attachment; filename=sales_report_".date('Y_m_d_H_i').".xls");  
            header("Pragma: no-cache"); 
            header("Expires: 0");
            
            $response = $this->sales_report_data();
            if($response){
                echo $response; 
            }
            return false;
     ...
     public function generate_sales_report_pdf()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $html = $this->sales_report_data();
            // echo $html;
            // die();

            $filename = 'sales_report_'. date('Y_m_d_H_i_s') . '_.pdf'; 
