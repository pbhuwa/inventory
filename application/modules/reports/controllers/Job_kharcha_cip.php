...
class Job_kharcha_cip extends CI_Controller
{
    function __construct()
    {
      
        parent::__construct();
        $this->load->Model('job_kharcha_cip_mdl');
    ...
    public function index()
    {
        ...
        $this->data['tab_type']='job_kharcha_cip';
        ...
        $this->template
            ...
            ->build('job_kharcha_cip/v_report_common', $this->data);
    }

    public function get_job_kharcha_cip_report(){
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $template = $this->job_kharcha_cip_data();
    ...
    public function job_kharcha_cip_data(){
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
           
            $this->data['excel_url'] = "reports/job_kharcha_cip/generate_job_kharcha_cip_report_excel";
            $this->data['pdf_url'] = "reports/job_kharcha_cip/generate_job_kharcha_cip_report_data_pdf";

            $this->data['report_title'] ="शिर्षगत जिन्सी पुँजिगत (CIP) खर्च विवरण";
            ...
            $this->data['get_leakage_report'] = $this->job_kharcha_cip_mdl->get_category_summary_rpt(17);
             $this->data['get_pipe_maintainance_report'] = $this->job_kharcha_cip_mdl->get_category_summary_rpt(18);
           ...
           if($this->location_ismain=='Y'){
            ...
            $this->data['fiscalyrs']=$fiscalyrs;
             // if(!empty($this->data['get_capital_exp_report'])){
                $template=$this->load->view('job_kharcha_cip/v_job_kharcha_cip_list', $this->data,true);
            // }else{
    ...
    public function generate_job_kharcha_cip_report_excel()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            header("Content-Type: application/xls");    
            header("Content-Disposition: attachment; filename=job_kharcha_cip_".date('Y_m_d_H_i').".xls");  
            header("Pragma: no-cache"); 
            header("Expires: 0");
            
            $response = $this->job_kharcha_cip_data();
            if($response){
                echo $response; 
            }
            return false;
     ...
     public function generate_job_kharcha_cip_report_data_pdf()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $html = $this->job_kharcha_cip_data();

            $filename = 'job_kharcha_cip_'. date('Y_m_d_H_i_s') . '_.pdf'; 
