...

{
...

	{
		...
		foreach ($data as $row) {
			...
			$array[$i]["rede_qty"] = sprintf('%g', $row->rede_qty);

			$array[$i]["rede_remqty"] = sprintf('%g',$row->rede_remqty);
	...
	public function get_headwise_account()
	{
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			$budgetid = $this->input->post('budgetid');
			 $this->db->select('acty_id, acty_accode, acty_acname');
...

	{
		...
		foreach ($data as $row) {
			...
			} else if (MODULES_UPDATE == 'Y' && $item_available_informed == 1 && $row->rema_postby == $this->userid) {
...

	{
		...
		foreach ($data as $row) {
			...
			$array[$i]['demand_cnt'] = sprintf('%g',$row->cnt);

			$array[$i]["demandqty"] = sprintf('%g',$row->demandqty);

			$array[$i]["stockqty"] = sprintf('%g',$row->stockqty);
		...
		try{
			...
			if (defined('RUN_API') && RUN_API == 'Y'){
				if (defined('API_CALL') && API_CALL == 'KUKL') {
					...
					// $location_code = 50;
					$api_response = $this->check_for_budget($location_code,$formatted_fiscalyear,$budget_code,$account_code);
					// print_r($data);die;
				}
			}
	...
	public function check_for_budget($office_code = 0,$fiscal_year = '',$costcenter_id = 0, $acc_code = 0 )
	{
