...
class New_issue_mdl extends CI_Model
{
	...
	public function insert_into_sale_master($items_id,$issue_qty,$salesmasteid,$rede_reqdetailid,$remarks,$s_no)
	{
		// $s_no = $this->input->post('s_no');
		// $rede_reqdetailid = $this->input->post('rede_reqdetailid');
...
// trde_trdeid'=>$mat_transdetailid));
   ...
   public function get_issue_status()
   {
	...
	$isreturn_cancel ='';
   	...
   	if($locationid)
   	{
   		$issue_cancel .= " sama_locationid = $locationid AND ";
   		$isreturn_cancel .= " rema_locationid = $locationid AND ";
   	}
   	if(!empty($frmdate) && !empty($todate)){
   		if(DEFAULT_DATEPICKER=='NP'){ 
		$issue_cancel .=" sama_billdatebs >='".$frmdate."' AND sama_billdatebs <='".$frmdate."' AND ";
		$isreturn_cancel .=" rema_returndatebs >='".$frmdate."' AND rema_returndatebs <='".$frmdate."' AND  ";
		}
		else{
			$issue_cancel .=" sama_billdatead >='".$frmdate."' AND sama_billdatead <='".$frmdate."' AND  ";
			$isreturn_cancel .=" rema_returndatead >='".$frmdate."' AND rema_returndatead <='".$frmdate."' AND  ";

		}
   	}
   	$sql="SELECT SUM(issue) issue, SUM(cancel)cancel, SUM(issue_ret) issue_ret, SUM(ret_cancel) ret_cancel
   	FROM(
		SELECT SUM( CASE WHEN sama_st = 'N' THEN 1 ELSE 0 END) issue, SUM( CASE WHEN sama_st = 'C' THEN 1 ELSE 0 END ) cancel, 0 as issue_ret, 0 as ret_cancel FROM xw_sama_salemaster WHERE $issue_cancel sama_salemasterid IS NOT NULL UNION SELECT 0 as issue, 0 as cancel, SUM(CASE WHEN rema_st='N' THEN 1 ELSE 0 END ) issuereturn, SUM(CASE WHEN rema_st='C' THEN 1 ELSE 0 END ) returncancel FROM xw_rema_returnmaster WHERE $isreturn_cancel rema_returnmasterid IS NOT NULL   )X";
...
$sql="SELECT * FROM
  ...
  WHERE it.itli_itemlistid=md.trde_itemsid AND mt.trma_received=1 AND md.trde_locationid='.$this->locationid.' AND mt.trma_fromdepartmentid='.$this->storeid.' ) as stockqty');
    ...
    public function get_requisition_data_from_salemasterid($id){
    	$this->db->select('r.rema_reqmasterid, r.rema_workdesc, r.rema_workplace');
    	$this->db->from('sama_salemaster s');
    	$this->db->join('rema_reqmaster r','r.rema_reqno = s.sama_requisitionno and r.rema_fyear=s.sama_fyear','left');
    	$this->db->where('sama_salemasterid',$id);
