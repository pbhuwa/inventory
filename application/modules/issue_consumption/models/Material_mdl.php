...
class Material_mdl extends CI_Model 
{
	...
	public $validate_settings_material = array( 
	  array('field' => 'maty_material', 'label' => 'Material Type', 'rules' => 'trim|required|xss_clean'),
       );                
...
public function get_dmaster_list()
	{
		$get = $_GET;
     	...
     	if(!empty($get['sSearch_0'])){
            $this->db->where("maty_materialtypeid like  '%".$get['sSearch_0']."%'  ");
        }

        if(!empty($get['sSearch_1'])){
            $this->db->where("maty_material like  '%".$get['sSearch_1']."%'  ");
        }
