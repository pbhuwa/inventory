        ...
        <div class="table-responsive">
            <table style="width:100%;" class="table dataTable dt_alt purs_table res_vert_table">
                ...
                if(!empty($transferred_details)):
                    ...
                    <?php endif; ?>
                    <th scope="col" width="10%"> <?php echo $this->lang->line('rate');?>
                    <th scope="col" width="10%"> <?php echo $this->lang->line('amount');?>
            ...
            <tbody id="orderBody">
                    ...
                    foreach($transferred_details as $tr_key=>$tr_row): 
                        ...
                        <tr class="orderrow <?php echo $stockTRClass;?>" id="orderrow_<?php echo $j;?>" data-id='<?php echo $j;?>'>
                            <td data-label="S.No.">
                                ...
                                <input type="hidden" class="eachsubtotal" name="eachsubtotal[]" id="eachsubtotal_<?php echo $tr_key+1; ?>" >
                            </td>
                            <td data-label="Code">
                                <div class="dis_tab"> 
                            ...
                            <td data-label="Transfer Quantity"> 
                                <input type="text" class="form-control calamt number required_field transfer_qty calculateamt" name="transfer_qty[]" id="transfer_qty_<?php echo $j;?>" value="<?php echo $transferring_qty; ?>" data-id='<?php echo $j;?>' >
                            </td>
                            <td>
                             <input type="text" class="form-control float tfde_unitprice calamt arrow_keypress" name="tfde_unitprice[]" data-fieldid="tfde_unitprice" id="tfde_unitprice_<?php echo $j;?>" data-id='<?php echo $j;?>' value="<?php echo !empty($tr_row->itli_purchaserate)?$tr_row->itli_purchaserate:''; ?>" readonly="true"> 

                         </td>
                         <td>
                            <?php $subtotal = ($tr_row->tfde_qty)*($tr_row->itli_purchaserate); ?>
                            <input type="text" name="tfde_totalamt[]" class="form-control eachtotalamt" value="<?php echo !empty($subtotal)?$subtotal:'0'; ?>"  id="tfde_totalamt_<?php echo $tr_key+1; ?>" readonly="true"> 
                        </td>
                ...
                <tr class="orderrow" id="orderrow_1" data-id='1'>
                    ...
                    <td data-label="Transfer Quantity"> 
                        <input type="text" class="form-control calamt required_field number transfer_qty calculateamt" name="transfer_qty[]"   id="transfer_qty_1" data-id='1' > 
                    </td>
                    <td>
                       <input type="text" class="form-control tfde_unitprice float calamt arrow_keypress" name="tfde_unitprice[]" data-fieldid="tfde_unitprice" id="tfde_unitprice_1" value="0"  data-id='1' placeholder="Rate">
                   </td>
                   <td>
                    <input type="text" name="tfde_totalamt[]" class="form-control eachtotalamt" value="0"  id="tfde_totalamt_1" readonly="true"> 
                </td>
                <td data-label="Remarks"> 
    ...
    <tbody>
        ...
        <tr class="resp_table_breaker">
          <td colspan="2" style="text-align: right;">
            ...
            <input type="text" class="form-control float" name="totalamount" id="totalamount" value="" readonly="true" />
        </td>
...
<script type="text/javascript">
   ...
   $(document).on('click','.btnAdd',function(){
    ...
    var rates = $('#tfde_unitprice_'+id).val();
     ...
     var trplusOne = $('.orderrow').length+1;
            ...
            setTimeout(function(){
              $(".orderrow").each(function(i,k) {
                ...
                $(this).find('.tfde_unitprice').attr("id","tfde_unitprice_"+vali);
                $(this).find('.tfde_unitprice').attr("data-id",vali);
                $(this).find('.totalamount').attr("id","totalamount_"+vali);
                $(this).find('.totalamount').attr("data-id",vali);

                

                $(this).find('.eachtotalamt').attr("id","tfde_totalamt_"+vali);
 ...
 $(document).on('click','.itemDetail',function(){
    ...
    var itemcode=$(this).data('itemcode');
        ...
        var tfde_unitprice=$(this).data('purrate');
        ...
        $('#tfde_unitprice_'+rowno).val(tfde_unitprice);
...
<script>
    ...
    $(document).on('keyup change','.calamt',function(){
        ...
        var rate=$('#tfde_unitprice_'+id).val();
        if(rate ==null || rate==' ')
        {
            rate=0;
        }
        ...
        $('#tfde_totalamt_'+id).val(valid_totalamt.toFixed(2));
        var stotal=0;
        var eachsubtotal=0;
