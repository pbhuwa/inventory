...
$user_list = [];
$operation_requested_to = !empty($requistion_data[0]->rema_reqto)?$requistion_data[0]->rema_reqto:'';
$operation_name = !empty($requistion_data[0]->rema_operation)?$requistion_data[0]->rema_operation:'';
?>
...
<!-- 
  ...
  <div class="list_c2 label_mw125">
    ...
    <?php 
        if (in_array($this->usergroup, $request_to_other_group_access)):
    ?>
    <div class="form-group">
    <form id="requestToForm" action="<?php echo base_url('issue_consumption/stock_requisition/request_for_operation');?>" method="POST">
        <input type="hidden" name="masterid" value="<?php echo !empty($requistion_data[0]->rema_reqmasterid)?$requistion_data[0]->rema_reqmasterid:'';  ?>">
            ...
            <div class="col-ms-12">
              ...
              <div class="row">
                <?php  
                    if (count($request_user_group) > 0) {
                        $location_array = [$this->locationid];
                        $main_location = $this->db->select('loca_locationid')->from('loca_location')->where('loca_ismain','Y')->where('loca_isactive','Y')->get()->row();
                        $main_locationid = 0;
                        if (!empty($main_location)) {
                            $main_locationid = $main_location->loca_locationid;
                            $location_array[] = $main_locationid;
                        }

                        $this->db->select('u.usma_userid, u.usma_username, ug.usgr_usergroup');
                        $this->db->from('usma_usermain as u');
                        $this->db->join('usgr_usergroup as ug','ug.usgr_usergroupid = u.usma_usergroupid');
                        $this->db->where_in('usgr_usergroupcode',$request_user_group);
                        $this->db->where('usma_isactive','1');
                        // $this->db->where('usma_locationid',$this->locationid);
                        $this->db->where_in('usma_locationid',$location_array);
                        $this->db->where('usma_orgid',$this->orgid);
                ...
                <div class="col-sm-3">
                    ...
                    <select name="request_to" class="form-control" id="request_to">
                        <?php 
                        if (!empty($user_list)): 
                            foreach ($user_list as $user):?>
                            <option value="<?php echo $user->usma_userid; ?>"><?php echo $user->usgr_usergroup.'-'.$user->usma_username;?></option>       
                        <?php 
                            endforeach;
                        endif; ?>
                    </select>
                ...
                <div class="col-sm-3">
                    ...
                    <?php 
                     if(defined('TWO_LEVEL_APPROVAL') && TWO_LEVEL_APPROVAL == 'Y'):?>
                ...
                <div class="col-sm-3" style="margin-top: 10px;">
                    ...
                    <button type="submit" class="btn btn-info savelist" data-operation="save" data-isdismiss="Y" data-isrefresh="Y"><?php echo $this->lang->line('save'); ?></button>
                </div>
    ...
    <?php endif;
        if (!empty($operation_data)): ?>
            ...
            <?php foreach($operation_data as $data): ?>
                <p><?php echo "Request for ".$data->aclo_status." sent to ".$data->usma_username;?></p>  
            <?php endforeach;?>    
            </div>
        <?php endif;?> 
    <?php 
      if (in_array($this->usergroup, $request_user_group)):
        if($this->userid == $operation_requested_to):
     ?>
